...
.. _program_listing_file_plugin_source_audio_BufferAudioPlayer.cpp:

Program Listing for File BufferAudioPlayer.cpp
==============================================

|exhale_lsh| :ref:`Return to documentation for file <file_plugin_source_audio_BufferAudioPlayer.cpp>` (``plugin/source/audio/BufferAudioPlayer.cpp``)
...
.. code-block:: cpp
   ...
   void BufferAudioPlayer::loadBuffer(const juce::AudioBuffer<float>& buffer, double sampleRate)
   {
       ...
       Log::info("BufferAudioPlayer: Loaded buffer - " +
                 juce::String(totalSamples) + " samples, " +
                 juce::String(bufferSampleRate) + "Hz, " +
   ...
   void BufferAudioPlayer::seekToPosition(double positionSeconds)
   {
       ...
       Log::debug("BufferAudioPlayer: Seeked to " + juce::String(positionSeconds, 2) + "s (sample " + juce::String((int)targetSample) + ")");
   }
           ...
           for (int sample = 0; sample < numSamples; ++sample)
           {
               ...
               if (readPosExact >= 0 && readPosExact < totalSamples)
               {
                   ...
                   if (readIndex < totalSamples - 1)
                   {
                       ...
                       sampleValue = inputChannelData[readIndex] * (1.0f - frac) +
                                     inputChannelData[readIndex + 1] * frac;
                   }
                   else
                   {
