...
.. _program_listing_file_plugin_source_ui_feed_Comment.cpp:

Program Listing for File Comment.cpp
====================================

|exhale_lsh| :ref:`Return to documentation for file <file_plugin_source_ui_feed_Comment.cpp>` (``plugin/source/ui/feed/Comment.cpp``)
...
.. code-block:: cpp
   ...
   void CommentsPanel::loadCommentsForPost(const juce::String& postId)
   {
       ...
       {
           Log::warn("CommentsPanel::loadCommentsForPost: Cannot load - postId empty or networkClient null");
           return;
       }
   ...
   void CommentsPanel::updateCommentsList()
   {
       ...
       for (const auto& comment : comments)
       {
           ...
           int rowHeight = comment.parentId.isNotEmpty() ?
               CommentRow::REPLY_ROW_HEIGHT : CommentRow::ROW_HEIGHT;
   ...
   void CommentsPanel::handleCommentLikeToggled(const Comment& comment, bool liked)
   {
       ...
       for (auto* row : commentRows)
       {
           if (row->getCommentId() == comment.id)
           {
               ...
               Log::debug("CommentsPanel::handleCommentLikeToggled: Optimistic update - new count: " + juce::String(newCount));
               break;
           }
       }
       ...
       if (liked)
       {
           ...
           networkClient->likeComment(comment.id, [this, commentId = comment.id](Outcome<juce::var> responseOutcome) {
               ...
               {
                   Log::warn("CommentsPanel::handleCommentLikeToggled: Like failed, reverting optimistic update");
                   // Revert on failure
                   for (auto* row : commentRows)
                   {
       ...
       else
       {
           ...
           networkClient->unlikeComment(comment.id, [this, commentId = comment.id](Outcome<juce::var> responseOutcome) {
               if (responseOutcome.isError())
               {
                   Log::warn("CommentsPanel::handleCommentLikeToggled: Unlike failed, reverting optimistic update");
                   // Revert on failure
                   for (auto* row : commentRows)
                   {
                       if (row->getCommentId() == commentId)
   ...
   {
       Log::info("CommentsPanel::handleCommentDeleted: Comment deletion result - success: " + juce::String(success ? "yes" : "no") +
                 ", commentId: " + commentId);
   ...
   void CommentsPanel::submitComment()
   {
       ...
       {
           Log::warn("CommentsPanel::submitComment: Cannot submit - networkClient null or postId empty");
           return;
       }
