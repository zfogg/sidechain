...
.. _program_listing_file_plugin_source_ui_social_FollowersList.cpp:

Program Listing for File FollowersList.cpp
==========================================

|exhale_lsh| :ref:`Return to documentation for file <file_plugin_source_ui_social_FollowersList.cpp>` (``plugin/source/ui/social/FollowersList.cpp``)
...
.. code-block:: cpp
   ...
   void FollowersList::loadList(const juce::String& userId, ListType type)
   {
       ...
       {
           Log::warn("FollowersList: Cannot load list - userId empty or network client null");
           return;
       }
   ...
   void FollowersList::handleUsersLoaded(bool success, const juce::var& usersData)
   {
       ...
       if (success && Json::isObject(usersData))
       {
           ...
           Log::info("FollowersList: Loaded " + juce::String(users.size()) + " " + typeStr + " (total: " + juce::String(totalCount) + ")");
           updateUsersList();
       }
       else
       {
