#include "StoriesFeedComponent.h"
...
//==============================================================================
StoriesFeedComponent::StoriesFeedComponent()
{
    // Start animation timer
    startTimerHz(60);

    Log::info("StoriesFeedComponent created");
}

StoriesFeedComponent::~StoriesFeedComponent()
{
    stopTimer();
    Log::info("StoriesFeedComponent destroyed");
}

//==============================================================================
void StoriesFeedComponent::paint(juce::Graphics& g)
{
    // Background
    ...
    {
        // Left fade
        juce::ColourGradient leftFade(StoryFeedColors::background, 0, 0,
                                      StoryFeedColors::background.withAlpha(0.0f), 20, 0, false);
...
void StoriesFeedComponent::resized()
{
...
void StoriesFeedComponent::mouseUp(const juce::MouseEvent& event)
{
...
void StoriesFeedComponent::mouseWheelMove(const juce::MouseEvent& /*event*/,
...
//==============================================================================
void StoriesFeedComponent::timerCallback()
{
...
//==============================================================================
void StoriesFeedComponent::loadStories()
{
    if (!networkClient)
    {
        Log::warn("StoriesFeedComponent: No network client set");
        return;
    }

    Log::info("StoriesFeedComponent: Loading stories...");

    networkClient->getStoriesFeed([this](Outcome<juce::var> result) {
        juce::MessageManager::callAsync([this, result]() {
            if (!result.isOk())
            {
                Log::warn("StoriesFeedComponent: Failed to load stories: " + result.getError());
                return;
            }
            ...
            if (response.hasProperty("stories"))
            {
                ...
                if (storiesArray)
                {
                    ...
                    setStories(stories);
                    Log::info("StoriesFeedComponent: Loaded " + juce::String(stories.size()) + " stories");
                }
            }
        });
    });
...
void StoriesFeedComponent::setStories(const std::vector<StoryData>& newStories)
{
    ...
    std::sort(userStoriesGroups.begin(), userStoriesGroups.end(),
              [](const UserStories& a, const UserStories& b) {
...
bool StoriesFeedComponent::hasOwnStory() const
{
...
//==============================================================================
void StoriesFeedComponent::drawCreateStoryCircle(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    if (currentUserAvatarUrl.isNotEmpty())
    {
        ...
        if (it != avatarCache.end() && it->second.isValid())
        {
            ...
            g.drawImage(it->second,
                        juce::Rectangle<float>(center.x - radius, center.y - radius, radius * 2, radius * 2),
    ...
    auto plusBounds = juce::Rectangle<float>(center.x + radius - plusSize * 0.6f,
                                              center.y + radius - plusSize * 0.6f,
                                              plusSize, plusSize);
...
void StoriesFeedComponent::drawStoryCircle(juce::Graphics& g, juce::Rectangle<int> bounds,
                                            const UserStories& userStories, int /*index*/)
{
    auto circleBounds = bounds.removeFromTop(CIRCLE_SIZE + RING_THICKNESS * 2);
    auto center = circleBounds.getCentre().toFloat();
    ...
    if (userStories.hasUnviewed)
    {
        ...
        juce::ColourGradient gradient(juce::Colour(0xff9c27b0), center.x, center.y - outerRadius,
                                      juce::Colour(0xff2196f3), center.x, center.y + outerRadius, false);
    ...
    if (userStories.stories.size() > 1)
    {
        ...
        auto badgeBounds = juce::Rectangle<float>(center.x + innerRadius - badgeSize * 0.5f,
                                                   center.y - innerRadius - badgeSize * 0.3f,
                                                   badgeSize, badgeSize);
        ...
        g.setColour(StoryFeedColors::textPrimary);
        g.setFont(10.0f);
        g.drawText(juce::String(userStories.stories.size()), badgeBounds.toNearestInt(),
...
int StoriesFeedComponent::calculateContentWidth() const
{
...
juce::Rectangle<int> StoriesFeedComponent::getCircleBounds(int index) const
{
...
void StoriesFeedComponent::loadAvatarImage(const juce::String& userId, const juce::String& avatarUrl)
{
