...
PianoRollComponent::PianoRollComponent()
{
    ...
    Log::debug("PianoRollComponent created");
}
...
//==============================================================================
void PianoRollComponent::setMIDIData(const juce::var& midiData)
{
    notes.clear();

    if (!midiData.isObject())
    {
        Log::warn("PianoRollComponent: Invalid MIDI data format");
        return;
    }
    ...
    Log::info("PianoRollComponent: Loaded " + juce::String(notes.size()) + " notes, " +
              juce::String(totalDuration, 2) + "s duration");

    repaint();
}

void PianoRollComponent::clearMIDIData()
{
    notes.clear();
    totalDuration = 0.0;
...
void PianoRollComponent::setPlaybackPosition(double positionSeconds)
{
    playbackPosition = positionSeconds;
    repaint();
}

void PianoRollComponent::setNoteRange(int lowNote, int highNote)
{
...
void PianoRollComponent::drawPianoKeys(juce::Graphics& g)
{
    ...
    for (int note = highNoteNumber; note >= lowNoteNumber; --note)
    {
        ...
        g.drawHorizontalLine(static_cast<int>(y + keyHeight), pianoKeyArea.getX(),
                              pianoKeyArea.getRight());
...
juce::String PianoRollComponent::getNoteName(int noteNumber) const
{
...
int PianoRollComponent::findNoteAt(juce::Point<int> position) const
{
...
juce::Colour PianoRollComponent::getNoteColor(const Note& note) const
{
    if (showChannels)
    {
...
juce::Colour PianoRollComponent::getChannelColor(int channel) const
{
...
void PianoRollComponent::parseMIDIEvents(const juce::var& events)
{
    if (!events.isArray())
        return;
