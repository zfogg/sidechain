...
{
    Log::info("HeaderComponent: Initializing header component");
    setSize(1000, HEADER_HEIGHT);
    Log::info("HeaderComponent: Initialization complete");
}

//==============================================================================
void HeaderComponent::setNetworkClient(NetworkClient* client)
{
    networkClient = client;
    Log::info("HeaderComponent::setNetworkClient: NetworkClient set " + juce::String(client != nullptr ? "(valid)" : "(null)"));
}

//==============================================================================
void HeaderComponent::setUserInfo(const juce::String& user, const juce::String& picUrl)
{
    Log::info("HeaderComponent::setUserInfo: Setting user info - username: " + user);
    username = user;

    // Only reload image if URL changed and we don't already have a cached image
    if (profilePicUrl != picUrl)
    {
        Log::debug("HeaderComponent::setUserInfo: Profile picture URL changed - old: " + profilePicUrl + ", new: " + picUrl);
        profilePicUrl = picUrl;

        // Only download if we don't have an image and URL is valid
        if (!cachedProfileImage.isValid() && Validate::isUrl(profilePicUrl))
        {
            Log::debug("HeaderComponent::setUserInfo: Loading profile image from URL");
            loadProfileImage(profilePicUrl);
        }
        else if (cachedProfileImage.isValid())
        {
            Log::debug("HeaderComponent::setUserInfo: Using cached profile image");
        }
        else if (!Validate::isUrl(profilePicUrl))
        {
            Log::warn("HeaderComponent::setUserInfo: Invalid profile picture URL: " + picUrl);
        }
    }
    else
    {
        Log::debug("HeaderComponent::setUserInfo: Profile picture URL unchanged, skipping reload");
    }
...
void HeaderComponent::setProfileImage(const juce::Image& image)
{
    if (image.isValid())
    {
        Log::info("HeaderComponent::setProfileImage: Setting profile image directly - size: " + juce::String(image.getWidth()) + "x" + juce::String(image.getHeight()));
        cachedProfileImage = image;
    }
    else
    {
        Log::warn("HeaderComponent::setProfileImage: Invalid image provided");
...
{
    Log::info("HeaderComponent::loadProfileImage: Loading profile image from: " + url);
    // Use Async::run to download image on background thread
    Async::run<juce::Image>(
        // Background work: download image
        [this, url]() -> juce::Image {
            Log::debug("HeaderComponent::loadProfileImage: Starting download on background thread");
            ...
            {
                Log::debug("HeaderComponent::loadProfileImage: Using NetworkClient for download");
                ...
                if (success)
                {
                    Log::debug("HeaderComponent::loadProfileImage: Download successful via NetworkClient - size: " + juce::String(imageData.getSize()) + " bytes");
                }
                else
                {
                    Log::warn("HeaderComponent::loadProfileImage: Download failed via NetworkClient - error: " + result.errorMessage);
                }
            }
            else
            {
                Log::debug("HeaderComponent::loadProfileImage: NetworkClient not available, using JUCE URL fallback");
                ...
                if (inputStream != nullptr)
                {
                    ...
                    if (success)
                    {
                        Log::debug("HeaderComponent::loadProfileImage: Download successful via JUCE URL - size: " + juce::String(imageData.getSize()) + " bytes");
                    }
                    else
                    {
                        Log::warn("HeaderComponent::loadProfileImage: Download failed via JUCE URL - empty data");
                    }
                }
                else
                {
                    Log::error("HeaderComponent::loadProfileImage: Failed to create input stream from URL");
                }
            }
            ...
            if (success)
            {
                ...
                if (image.isValid())
                {
                    Log::debug("HeaderComponent::loadProfileImage: Image decoded successfully - size: " + juce::String(image.getWidth()) + "x" + juce::String(image.getHeight()));
                    return image;
                }
                else
                {
                    Log::error("HeaderComponent::loadProfileImage: Failed to decode image from downloaded data");
                }
            }
        ...
        [this, url](const juce::Image& image) {
            if (image.isValid())
            {
                Log::info("HeaderComponent::loadProfileImage: Profile image loaded successfully from: " + url);
                cachedProfileImage = image;
                repaint();
            }
            else
            {
                Log::error("HeaderComponent::loadProfileImage: Failed to load profile image from: " + url);
            }
        }
    );
}
...
{
    Log::debug("HeaderComponent::resized: Component resized to " + juce::String(getWidth()) + "x" + juce::String(getHeight()));
...
void HeaderComponent::mouseUp(const juce::MouseEvent& event)
{
    ...
    Log::debug("HeaderComponent::mouseUp: Mouse clicked at (" + juce::String(pos.x) + ", " + juce::String(pos.y) + ")");

    if (getLogoBounds().contains(pos))
    {
        Log::info("HeaderComponent::mouseUp: Logo clicked");
        if (onLogoClicked)
            onLogoClicked();
        else
            Log::warn("HeaderComponent::mouseUp: Logo clicked but callback not set");
    }
    else if (getSearchButtonBounds().contains(pos))
    {
        Log::info("HeaderComponent::mouseUp: Search button clicked");
        if (onSearchClicked)
            onSearchClicked();
        else
            Log::warn("HeaderComponent::mouseUp: Search clicked but callback not set");
    }
    else if (getRecordButtonBounds().contains(pos))
    {
        Log::info("HeaderComponent::mouseUp: Record button clicked");
        if (onRecordClicked)
            onRecordClicked();
        else
    ...
    else if (getProfileBounds().contains(pos))
    {
        ...
        if (onProfileClicked)
            ...
            Log::warn("HeaderComponent::mouseUp: Profile clicked but callback not set");
    }
}
...
void HeaderComponent::drawRecordButton(juce::Graphics& g, juce::Rectangle<int> bounds)
{
...
juce::Rectangle<int> HeaderComponent::getRecordButtonBounds() const
{
