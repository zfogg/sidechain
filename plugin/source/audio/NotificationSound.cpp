...
bool NotificationSound::tryPlaySystemSound()
{
    juce::File soundFile;
        ...
        juce::StringArray soundNames = { "Glass.aiff", "Basso.aiff", "Blow.aiff", "Bottle.aiff", 
                                         "Frog.aiff", "Funk.aiff", "Hero.aiff", "Morse.aiff",
        ...
        juce::StringArray soundNames = { 
            "Windows Notify.wav",           // Windows 10/11 default notification
        ...
        for (const auto& path : searchPaths)
        {
            juce::File searchDir(path.startsWith("~") ? 
...
{
public:
    BufferAudioSource(const juce::AudioBuffer<float>& buf) 
    ...
    void getNextAudioBlock(const juce::AudioSourceChannelInfo& info) override
    {
        ...
        {
            info.buffer->copyFrom(ch, info.startSample, buffer, 
                                 ch % buffer.getNumChannels(), position, samplesToCopy);
        }
        
        if (samplesToCopy < info.numSamples)
            info.buffer->clear(info.startSample + samplesToCopy, 
...
void NotificationSound::playGeneratedBeep()
{
    ...
    // Generate a simple beep: 800Hz sine wave, 100ms duration, 44.1kHz sample rate
...
void NotificationSound::scheduleCleanup()
{
    ...
    juce::Timer::callAfterDelay(150, []() {
        if (audioSourcePlayer)
            audioSourcePlayer->setSource(nullptr);
        // beepSource will be cleaned up when playBeep() is called again or on shutdown
    });
}
