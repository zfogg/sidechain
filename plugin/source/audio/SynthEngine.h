...
public:
    ...
    void setWaveform(Waveform waveform) { currentWaveform.store(static_cast<int>(waveform)); }

    /** Get current waveform */
    Waveform getWaveform() const { return static_cast<Waveform>(currentWaveform.load()); }

    /** Set ADSR envelope parameters */
    void setADSR(float attack, float decay, float sustain, float release);

    /** Set filter cutoff frequency (Hz) */
    void setFilterCutoff(float cutoff) { filterCutoff.store(cutoff); }

    /** Get filter cutoff */
    float getFilterCutoff() const { return filterCutoff.load(); }

    /** Set filter resonance (0-1) */
    void setFilterResonance(float resonance) { filterResonance.store(resonance); }
    ...
    void setDetune(float cents) { detuneAmount.store(cents); }

    /** Get detune amount */
    float getDetune() const { return detuneAmount.load(); }
