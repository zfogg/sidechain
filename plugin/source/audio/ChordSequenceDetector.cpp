...
void ChordSequenceDetector::processMIDI(const juce::MidiBuffer& midiMessages, double sampleRate)
{
    ...
    if (notesChanged && currentlyHeldNotes.size() >= 3)
    {
        ...
        if (detected.type != ChordType::Unknown &&
            !(detected == lastDetectedChord))
        {
            {
                juce::ScopedLock lock(chordLock);
...
//==============================================================================
ChordSequenceDetector::ChordType ChordSequenceDetector::identifyChordType(
    const std::set<int>& pitchClasses, int& outRoot)
{
    ...
    for (int root = 0; root < 12; root++)
    {
        ...
        {
            outRoot = root;
            if (intervals.count(11)) return ChordType::Major7;
            if (intervals.count(10)) return ChordType::Dominant7;
        ...
        {
            outRoot = root;
            if (intervals.count(10)) return ChordType::Minor7;
...
void ChordSequenceDetector::triggerUnlock(UnlockSequence& sequence)
{
    ...
    {
        juce::MessageManager::callAsync([callback = sequence.onUnlock]()
        {
            callback();
        });
    }
...
ChordSequenceDetector::UnlockSequence ChordSequenceDetector::createBasicSynthSequence(
...
ChordSequenceDetector::UnlockSequence ChordSequenceDetector::createAdvancedSynthSequence(
    std::function<void()> onUnlock)
{
    UnlockSequence seq;
...
ChordSequenceDetector::UnlockSequence ChordSequenceDetector::createSecretSequence(
    std::function<void()> onUnlock)
{
    UnlockSequence seq;
