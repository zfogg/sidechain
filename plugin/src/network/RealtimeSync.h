...
public:
  ...
  RealtimeSync(int clientId, const juce::String &documentId)
      : clientId_(clientId), documentId_(documentId), isSynced_(true), operationCounter_(0) {}
  ...
  void handleRemoteOperation(const std::shared_ptr<Operation> &operation) {
...

    {
      ...
      for (const auto &localOp : pending) {
        auto [_, xformed] = Sidechain::Util::CRDT::OperationalTransform::transform(localOp, transformed);
        transformed = xformed;
      }
   ...
   * Get document ID
   */
  const juce::String &getDocumentId() const {
    return documentId_;
  }
...
/**
 * WebSocketMessageHandler - Handles incoming WebSocket messages for OT
 * operations
 *
 * Message format (JSON):
 * {
...
public:
   ...
   */
  static void registerSync(const juce::String &documentId, std::shared_ptr<RealtimeSync> sync) {
