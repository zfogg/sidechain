...
void FeedStore::toggleArchive(const juce::String &postId, bool archived) {
  ...
  if (archived) {
    networkClient->archivePost(postId, [postId](Outcome<juce::var> result) {
      if (result.isOk()) {
    ...
    networkClient->unarchivePost(postId, [postId](Outcome<juce::var> result) {
      if (result.isOk()) {
...
void FeedStore::toggleMute(const juce::String &userId, bool willMute) {
  ...
  if (willMute) {
    networkClient->muteUser(userId, [userId](Outcome<juce::var> result) {
      if (result.isOk()) {
    ...
    networkClient->unmuteUser(userId, [userId](Outcome<juce::var> result) {
      if (result.isOk()) {
...
void FeedStore::toggleBlock(const juce::String &userId, bool willBlock) {
  ...
  if (willBlock) {
    networkClient->blockUser(userId, [userId](Outcome<juce::var> result) {
      if (result.isOk()) {
    ...
    networkClient->unblockUser(userId, [userId](Outcome<juce::var> result) {
      if (result.isOk()) {
...
juce::File FeedStore::getCacheFile(FeedType feedType) const {
  ...
  case FeedType::ForYou:
    ...
    break;
  case FeedType::Popular:
    filename = "feed_popular.json";
    break;
  case FeedType::Latest:
    filename = "feed_latest.json";
    break;
  case FeedType::Discovery:
    filename = "feed_discovery.json";
    break;
  case FeedType::TimelineAggregated:
    filename = "feed_timeline_agg.json";
    break;
  case FeedType::TrendingAggregated:
    filename = "feed_trending_agg.json";
    break;
  case FeedType::NotificationAggregated:
    filename = "feed_notif_agg.json";
    break;
  case FeedType::UserActivityAggregated:
    filename = "feed_user_activity_agg.json";
    break;
  }
...
void FeedStore::schedulePopularFeedWarmup() {
  ...
  cacheWarmer->scheduleWarmup(
      "timeline", [this]() { warmTimeline(); },
      10 // High priority
  );

  cacheWarmer->scheduleWarmup(
      "trending", [this]() { warmTrending(); },
      20 // Medium priority
  );

  cacheWarmer->scheduleWarmup(
      "user_posts", [this]() { warmUserPosts(); },
      30 // Lower priority
  );
