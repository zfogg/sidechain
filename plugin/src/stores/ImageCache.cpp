...
// CFNetwork has a race condition in its caching mechanism when multiple
// concurrent requests happen. This mutex serializes URL downloads to work
// around it.
...
void downloadImage(const juce::String &url, int targetWidth, int targetHeight) {
  // Use Async::run to download image on background thread and deliver to
  // message thread
      ...
      [url, targetWidth, targetHeight]() -> juce::Image {
        ...
        try {
          ...
          if (networkClient != nullptr) {
            ...
            auto result = networkClient->makeAbsoluteRequestSync(url, "GET", juce::var(), false,
                                                                 juce::StringPairArray(), &binaryData);
            if (result.success && binaryData.getSize() > 0) {
              data = binaryData;
              success = true;
            }
          } else {
            // Fallback to JUCE URL - serialize to avoid CFNetwork caching
            // crashes CFNetwork has a race condition when multiple concurrent
            // requests try to cache
            ...
            if (!loadedImage.isValid()) {
              ...
              if (targetWidth > 0 || targetHeight > 0) {
                ...
                if (newWidth != loadedImage.getWidth() || newHeight != loadedImage.getHeight()) {
                  loadedImage = loadedImage.rescaled(newWidth, newHeight, juce::Graphics::highResamplingQuality);
                }
              }
            }
          }
...
void drawCircularAvatar(juce::Graphics &g, juce::Rectangle<int> bounds, const juce::Image &image,
                        const juce::String &initials, juce::Colour bgColor, juce::Colour textColor, float fontSize) {
...
// The backend returns JSON with the actual image URL, which we then download
// via HTTPS
...
void loadAvatarForUser(const juce::String &userId, ImageCallback callback, int width, int height) {
  ...
  juce::String apiUrl = juce::String(Constants::Endpoints::DEV_BASE_URL) + Constants::Endpoints::API_VERSION +
  ...
  Async::runVoid([apiUrl, userId, cacheKey, callback, width, height]() {
    ...
    if (networkClient != nullptr) {
      auto result =
          networkClient->makeAbsoluteRequestSync(imageUrl, "GET", juce::var(), false, juce::StringPairArray(), &data);
    ...
    if (success && data.getSize() > 0) {
      ...
      if (loadedImage.isValid()) {
        ...
        Log::debug("ImageLoader: Avatar loaded for user " + userId + " (" + juce::String(loadedImage.getWidth()) + "x" +
