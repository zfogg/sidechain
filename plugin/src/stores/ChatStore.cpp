...
void ChatStore::setupEventHandlers() {
  ...
  streamChatClient->setMessageReceivedCallback(
      [this](const StreamChatClient::Message &message, const juce::String &channelId) {
        handleMessageReceived(message, channelId);
      });

  streamChatClient->setTypingCallback([this](const juce::String &userId, bool isTyping) {
  ...
  streamChatClient->setConnectionStatusCallback(
      [this](StreamChatClient::ConnectionStatus status) { handleConnectionStatusChanged(status); });

  streamChatClient->setPresenceChangedCallback(
      [this](const StreamChatClient::UserPresence &presence) { updateUserPresence(presence.userId, presence); });
}

//==============================================================================
// Authentication

void ChatStore::setAuthentication(const juce::String &token, const juce::String &apiKey, const juce::String &userId) {
...
void ChatStore::createDirectChannel(const juce::String &targetUserId) {
  ...
  streamChatClient->createDirectChannel(targetUserId, [this, targetUserId](Outcome<StreamChatClient::Channel> result) {
    if (result.isOk()) {
...
void ChatStore::createGroupChannel(const juce::String &channelId, const juce::String &name,
  ...
  streamChatClient->createGroupChannel(channelId, name, memberIds, [this](Outcome<StreamChatClient::Channel> result) {
    if (result.isOk()) {
      auto channel = result.getValue();
...
void ChatStore::deleteChannel(const juce::String &channelId) {
  ...
  if (streamChatClient) {
    streamChatClient->deleteChannel(channelType, channelId, [this, channelId](Outcome<void> result) {
      if (result.isOk()) {
        ...
        Util::logError("ChatStore", "Failed to delete channel: " + result.getError(), "channelId=" + channelId);
      }
    });
  }
}
...
void ChatStore::leaveChannel(const juce::String &channelId) {
  ...
  if (streamChatClient) {
    streamChatClient->leaveChannel(channelType, channelId, [this, channelId](Outcome<void> result) {
      if (result.isOk()) {
        ...
        Util::logError("ChatStore", "Failed to leave channel: " + result.getError(), "channelId=" + channelId);
      }
    });
  }
}
...
void ChatStore::selectChannel(const juce::String &channelId) {
  ...
  Log::debug("ChatStore::selectChannel - Looking for channel in map. Found=" +
             juce::String(channelFound ? "YES" : "NO"));

  if (it != currentState.channels.end()) {
    Log::debug("ChatStore::selectChannel - Channel found. Has " + juce::String(it->second.messages.size()) +
               " messages");
    if (it->second.messages.empty()) {
      Log::debug("ChatStore::selectChannel - Messages empty, calling loadMessages");
      loadMessages(channelId);
    } else {
      Log::debug("ChatStore::selectChannel - Messages not empty, skipping "
                 "loadMessages");
    }
  } else {
    Log::debug("ChatStore::selectChannel - Channel not found in map, will not "
               "auto-load messages");
  }
...
void ChatStore::loadMessages(const juce::String &channelId, int limit) {
  ...
  if (!streamChatClient || !isAuthenticated()) {
    Log::error("ChatStore::loadMessages - NOT AUTHENTICATED: streamChatClient=" +
               juce::String(streamChatClient != nullptr ? "SET" : "NULL") +
  ...
  // and the user opens a channel before ChatStore has loaded its own channel
  // list
  ...
  if (it != currentState.channels.end()) {
    ...
    Log::debug("ChatStore::loadMessages - Channel not in map, creating "
  ...
  Log::info("ChatStore::loadMessages - About to call queryMessages with type=" + channelType + " id=" + channelId +
  ...
  streamChatClient->queryMessages(
      channelType, channelId, limit, 0, [this, channelId](Outcome<std::vector<StreamChatClient::Message>> result) {
        Log::info("ChatStore::loadMessages - queryMessages CALLBACK: channelId=" + channelId +
                  " isOk=" + juce::String(result.isOk() ? "YES" : "NO"));
        if (result.isOk()) {
          Log::info("ChatStore::loadMessages - queryMessages succeeded, got " + juce::String(result.getValue().size()) +
                    " messages");
...
void ChatStore::sendMessage(const juce::String &channelId, const juce::String &text) {
  ...
  Log::debug(
      "ChatStore::sendMessage - streamChatClient: " + juce::String(streamChatClient != nullptr ? "SET" : "NULL") +
      ", isAuthenticated: " + juce::String(isAuthenticated() ? "YES" : "NO"));

  if (!streamChatClient || !isAuthenticated()) {
    Log::error("ChatStore::sendMessage - Cannot send message - streamChatClient=" +
               juce::String(streamChatClient != nullptr ? "SET" : "NULL") +
               ", authenticated=" + juce::String(isAuthenticated() ? "YES" : "NO"));
  ...
  optimisticUpdate(
      [channelId, text, tempId, userId](ChatStoreState &state) {
        Log::debug("ChatStore::sendMessage - Optimistic callback firing for "
                   "channelId: " +
                   channelId);
        auto &channel = state.channels[channelId];
        ...
        Log::debug("ChatStore::sendMessage - Added temp message. Channel now has " +
                   juce::String(channel.messages.size()) + " messages");
      },
      ...
      [channelId, tempId](const juce::String &error) {
        Util::logError("ChatStore", "Failed to send message: " + error, "channelId=" + channelId + " tempId=" + tempId);
      });
}
...
void ChatStore::sendMessageWithAudio(const juce::String &channelId, const juce::String &text,
  ...
  streamChatClient->sendMessageWithAudio(
      channelType, channelId, text, audioBuffer, sampleRate,
      [this, channelId](Outcome<StreamChatClient::Message> result) {
        if (result.isOk()) {
          handleMessageSent(channelId, result.getValue());
        } else {
          Util::logError("ChatStore", "Failed to send audio message: " + result.getError(), "channelId=" + channelId);
        }
      });
}
...
void ChatStore::deleteMessage(const juce::String &channelId, const juce::String &messageId) {
  ...
  streamChatClient->deleteMessage(
      channelType, channelId, messageId, [this, channelId, messageId](Outcome<void> result) {
        if (result.isOk()) {
          updateState([channelId, messageId](ChatStoreState &state) {
            auto &channel = state.channels[channelId];
            auto msgIt =
                std::find_if(channel.messages.begin(), channel.messages.end(),
          ...
          Util::logError("ChatStore", "Failed to delete message: " + result.getError(), "messageId=" + messageId);
        }
      });
}
...
void ChatStore::sharePostToChannels(const juce::String &postId, const std::vector<juce::String> &channelIds,
  ...
  Util::logDebug("ChatStore", "Sharing post to channels",
                 "postId=" + postId + " channelCount=" + juce::String((int)channelIds.size()));
...
void ChatStore::shareStoryToChannels(const juce::String &storyId, const std::vector<juce::String> &channelIds,
  ...
  Util::logDebug("ChatStore", "Sharing story to channels",
                 "storyId=" + storyId + " channelCount=" + juce::String((int)channelIds.size()));
...
void ChatStore::sendMessageWithSharedContent(const juce::String &channelId, const juce::String &text,
  ...
  streamChatClient->sendMessage(
      channelType, channelId, text, messageData, [channelId, text](Outcome<StreamChatClient::Message> result) {
        if (!result.isOk()) {
...
void ChatStore::markAsRead(const juce::String &channelId) {
  ...
  streamChatClient->markChannelRead(channelType, channelId, [this, channelId](Outcome<void> result) {
    if (result.isOk()) {
      updateState([channelId](ChatStoreState &state) {
        auto &channel = state.channels[channelId];
...
void ChatStore::updateUserPresence(const juce::String &userId, const StreamChatClient::UserPresence &presence) {
  updateState([userId, presence](ChatStoreState &state) { state.userPresence[userId] = presence; });
}
...
void ChatStore::handleMessageSent(const juce::String &channelId, const StreamChatClient::Message &message) {
  Log::info("ChatStore::handleMessageSent - Received message for channelId: " + channelId +
  ...
  updateState([channelId, message](ChatStoreState &state) {
    ...
    if (it != state.channels.end()) {
      auto &channel = it->second;
      Log::debug("ChatStore::handleMessageSent - Found channel. Currently has " +
                 juce::String(channel.messages.size()) + " messages");

      // Find and replace temporary message with real one
      auto msgIt = std::find_if(channel.messages.begin(), channel.messages.end(),
                                [](const StreamChatClient::Message &msg) { return msg.id.startsWith("temp_"); });

      if (msgIt != channel.messages.end()) {
        Log::debug("ChatStore::handleMessageSent - Replacing temp message with "
                   "real one");
        *msgIt = message;
      } else {
        // Not found, just add it
        Log::debug("ChatStore::handleMessageSent - Temp message not found, "
                   "adding new message");
        channel.messages.push_back(message);
      }
      Log::debug("ChatStore::handleMessageSent - Channel now has " + juce::String(channel.messages.size()) +
                 " messages");
    } else {
...
void ChatStore::editChannelDescription(const juce::String &channelId, const juce::String &newDescription) {
  ...
  updateState([channelId, newDescription, this](ChatStoreState &state) {
    ...
    Util::logDebug("ChatStore", "Queued description operation", "timestamp=" + juce::String(operation->timestamp));
  });
...
void ChatStore::sendOperationToServer(const juce::String &channelId,
  ...
  // TODO: Implement endpoint in backend: POST
  // /api/v1/channels/{channelId}/description-operation
...
void ChatStore::handleRemoteOperation(
    const juce::String &channelId, const std::shared_ptr<Util::CRDT::OperationalTransform::Operation> &remoteOperation,
    int remoteClientId) {
  if (!remoteOperation)
    return;
