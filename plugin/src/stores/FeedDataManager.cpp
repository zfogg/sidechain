...
void FeedDataManager::fetchFeed(FeedType feedType, int limit, int offset, FeedCallback callback) {
  juce::String feedTypeStr = feedType == FeedType::Timeline   ? "Timeline"
                             ...
                             : feedType == FeedType::Trending ? "Trending"
                                                              : "ForYou";
  Log::debug("FeedDataManager: Fetching feed - type: " + feedTypeStr + ", limit: " + juce::String(limit) +
  ...
  if (offset == 0 && isCacheValid(feedType)) {
    ...
    if (!cached.posts.isEmpty()) {
      Log::info("FeedDataManager: Using cached feed - type: " + feedTypeStr +
                ", posts: " + juce::String(cached.posts.size()));
      // Use cached data
      juce::MessageManager::callAsync([callback, cached]() { callback(cached); });
      return;
    }
  }
...
void FeedDataManager::performFetch(FeedType feedType, int limit, int offset, FeedCallback callback) {
  ...
  juce::String feedTypeStr = feedType == FeedType::Timeline   ? "Timeline"
                             ...
                             : feedType == FeedType::Trending ? "Trending"
                                                              : "ForYou";
  Log::info("FeedDataManager: Performing network fetch - type: " + feedTypeStr + ", limit: " + juce::String(limit) +
            ", offset: " + juce::String(offset));
...
//==============================================================================
void FeedDataManager::handleFetchResponse(const juce::var &feedData, FeedType feedType, int limit, int offset,
  ...
  juce::String feedTypeStr = feedType == FeedType::Timeline   ? "Timeline"
                             ...
                             : feedType == FeedType::Trending ? "Trending"
                                                              : "ForYou";
  Log::info("FeedDataManager: Fetch response received - type: " + feedTypeStr +
            ", posts: " + juce::String(feedResponse.posts.size()) +
            ", hasMore: " + juce::String(feedResponse.hasMore ? "true" : "false"));
  ...
  juce::MessageManager::callAsync([callback, feedResponse]() { callback(feedResponse); });
}
...
void FeedDataManager::handleFetchError(const juce::String &error, FeedCallback callback) {
  ...
  juce::MessageManager::callAsync([callback, errorResponse]() { callback(errorResponse); });
}
...
FeedResponse FeedDataManager::parseJsonResponse(const juce::var &json) {
  ...
  // Parse activities array (from backend /api/feed/timeline or
  // /api/feed/global)
  ...
  // Parse pagination info - check meta object first (new unified endpoint
  // format)
...
juce::File FeedDataManager::getCacheFile(FeedType feedType) const {
  ...
  switch (feedType) {
  case FeedType::Timeline:
    filename = "feed_timeline.json";
    break;
  case FeedType::Global:
    filename = "feed_global.json";
    break;
  case FeedType::Trending:
    filename = "feed_trending.json";
    break;
  case FeedType::ForYou:
    filename = "feed_foryou.json";
    break;
  default:
    filename = "feed_unknown.json";
    break;
  }
