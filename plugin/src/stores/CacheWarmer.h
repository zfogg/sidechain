...
public:
  ...
  CacheWarmer()
      : defaultTTL_(3600), isRunning_(false), isOnline_(true), operationsQueued_(0), operationsCompleted_(0) {}
  ...
  void scheduleWarmup(const juce::String &key, OperationCallback operation, int priority = 50) {
...

    {
      ...
      std::sort(operations_.begin(), operations_.end(),
                [](const WarmupOperation &a, const WarmupOperation &b) { return a.priority < b.priority; });
    }
...
private:
  ...
  void processNextOperation() {
    ...
    try {
      ...
      juce::Logger::getCurrentLogger()->writeToLog("CacheWarmer operation failed: " + juce::String(e.what()));
      operationsCompleted_++;
      processNextOperation();
    }
  }
