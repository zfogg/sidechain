 ...
 *
 * Named ImageLoader instead of ImageCache to avoid conflict with
 * juce::ImageCache.
 *
 * Features:
 ...
 */
void load(const juce::String &url, ImageCallback callback, int width = 0, int height = 0);
...
/**
 * Set NetworkClient for HTTP requests (optional - falls back to JUCE URL if not
 * set).
 */
 ...
 *
 * Use this instead of load() for avatar images to ensure cross-platform
 * compatibility.
 *
 ...
 * @param height   Optional: resize to this height (0 = original size)
 */
void loadAvatarForUser(const juce::String &userId, ImageCallback callback, int width = 0, int height = 0);
...
void drawCircularAvatar(juce::Graphics &g, juce::Rectangle<int> bounds, const juce::Image &image,
                        const juce::String &initials, juce::Colour bgColor, juce::Colour textColor,
