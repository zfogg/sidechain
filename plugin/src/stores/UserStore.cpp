...
void UserStore::updateProfile(const juce::String &username, const juce::String &displayName, const juce::String &bio) {
  if (!networkClient) {
  ...
  optimisticUpdate(
      ...
      [this, username, displayName, bio](auto callback) {
        networkClient->updateUserProfile(username, displayName, bio, [callback](Outcome<juce::var> result) {
          callback(result.isOk(), result.isOk() ? "" : result.getError());
        });
      },
      [](const juce::String &error) { Util::logError("UserStore", "Failed to update profile: " + error); });
}
...
void UserStore::setLocalPreviewImage(const juce::File &imageFile) {
  ...
  Async::run<juce::Image>([imageFile]() { return juce::ImageFileFormat::loadFrom(imageFile); },
                          [this](const juce::Image &image) {
                            if (image.isValid()) {
                              updateState([image](UserState &state) { state.profileImage = image; });
                            }
                          });
}
...
void UserStore::setNotificationSoundEnabled(bool enabled) {
  ...
  updateState([enabled](UserState &state) { state.notificationSoundEnabled = enabled; });
...
void UserStore::setOSNotificationsEnabled(bool enabled) {
  ...
  updateState([enabled](UserState &state) { state.osNotificationsEnabled = enabled; });
...
void UserStore::changeUsername(const juce::String &newUsername) {
  ...
  networkClient->changeUsername(newUsername, [this, newUsername](Outcome<juce::var> result) {
    juce::MessageManager::callAsync([this, newUsername, result]() {
      if (result.isOk()) {
        updateState([newUsername](UserState &state) { state.username = newUsername; });
...
void UserStore::updateProfileComplete(const juce::String &displayName, const juce::String &bio,
...
void UserStore::uploadProfilePicture(const juce::File &imageFile) {
  ...
  // Show loading state
  updateState([](UserState &state) { state.isLoadingImage = true; });
    ...
    juce::MessageManager::callAsync([this, result]() {
      if (result.isOk()) {
        ...
        if (s3Url.isNotEmpty()) {
          ...
          updateState([](UserState &state) { state.isLoadingImage = false; });
        }
      } else {
