...
public:
     ...
     */
    explicit Store(const State& initialState = State())
...
public:
    ...
    explicit ScopedSubscription(std::function<void()> unsubscriber)
        : unsubscribe(std::move(unsubscriber))
    {
    }
    ...
    bool isActive() const { return unsubscribe != nullptr; }
