...
void UserDataStore::setProfilePictureUrl(const juce::String &url) {
  ...
  if (Validate::isUrl(url)) {
    // Use proxy endpoint instead of direct S3 URL to work around JUCE SSL
    // issues on Linux
...
juce::String UserDataStore::getProxyUrl() const {
  ...
  if (userId.isNotEmpty() && networkClient != nullptr) {
    // Construct proxy URL using constant:
    // /api/v1/users/{userId}/profile-picture?proxy=true The ?proxy=true tells
    // the backend to return actual image bytes instead of JSON
    return juce::String(Constants::Endpoints::DEV_BASE_URL) + Constants::Endpoints::API_VERSION + "/users/" + userId +
           "/profile-picture?proxy=true";
  }
  return "";
}
 ...
 * Downloads the image asynchronously and updates cachedProfileImage when
 * complete
...
void UserDataStore::downloadProfileImage(const juce::String &url) {
  ...
  // Capture auth token and current profile picture URL for the background
  // thread Note: We capture profilePictureUrl (not the download url) because we
  // may be using a proxy URL for download, but want to verify the original URL
  // hasn't changed
  ...
  std::function<juce::Image()> workFunc = [client, url, token]() -> juce::Image {
    ...
    if (client != nullptr) {
      ...
      auto inputStream =
          juceUrl.createInputStream(juce::URL::InputStreamOptions(juce::URL::ParameterHandling::inAddress)
  ...
  std::function<void(juce::Image)> completeFunc = [this, originalUrl](const juce::Image &image) {
    ...
    // (we compare against originalUrl, not the download URL which may be a
    // proxy)
    if (profilePictureUrl == originalUrl) {
      cachedProfileImage = image;

      if (cachedProfileImage.isValid()) {
        Log::info("UserDataStore: Image loaded OK (" + juce::String(cachedProfileImage.getWidth()) + "x" +
      ...
      Log::warn("UserDataStore: URL changed during download, ignoring (orig=" + originalUrl +
 ...
 * @param callback Optional callback called when fetch completes (true if
 * successful)
 */
void UserDataStore::fetchUserProfile(std::function<void(bool success)> callback) {
  if (!networkClient || authToken.isEmpty()) {
    Log::warn("UserDataStore: Cannot fetch profile - no network client or auth "
              "token");
    if (callback)
      callback(false);
    return;
  ...
  networkClient->get("/api/v1/users/me", [this, callback](Outcome<juce::var> result) {
    juce::MessageManager::callAsync([this, result, callback]() {
      if (result.isOk() && Json::isObject(result.getValue())) {
        ...
        // Use avatar_url which falls back to oauth_profile_picture_url if
        // profile_picture_url is empty This ensures users with OAuth login
        // (Google, GitHub, etc.) see their profile picture
        juce::String newPicUrl = Json::getString(response, "avatar_url");

        Log::info("UserDataStore: Profile fetched - username: " + username + ", avatarUrl: " + newPicUrl);
