...
enum class FeedType {
  ...
  Discovery, // Discovery feed from Gorse (blends popular, latest, and
             // personalized)
...
struct SingleFeedState {
  ...
  bool operator==(const SingleFeedState &other) const {
    return posts.size() == other.posts.size() && isLoading == other.isLoading && isRefreshing == other.isRefreshing &&
           hasMore == other.hasMore && offset == other.offset && error == other.error &&
           lastUpdated == other.lastUpdated && isSynced == other.isSynced;
  }
};

/**
 * AggregatedFeedState - State for aggregated feeds (groups instead of flat
 * posts)
 */
struct AggregatedFeedState {
  ...
  bool operator==(const AggregatedFeedState &other) const {
    return groups.size() == other.groups.size() && isLoading == other.isLoading && isRefreshing == other.isRefreshing &&
           hasMore == other.hasMore && offset == other.offset && error == other.error &&
           lastUpdated == other.lastUpdated && isSynced == other.isSynced;
  }
};
...
struct FeedStoreState {
  ...
  bool operator==(const FeedStoreState &other) const {
    return feeds == other.feeds && aggregatedFeeds == other.aggregatedFeeds && currentFeedType == other.currentFeedType;
  }
};
 ...
 *
 *   // Optimistic like
 *   feedStore.toggleLike(postId);  // Updates UI immediately, syncs in
 * background
 */
