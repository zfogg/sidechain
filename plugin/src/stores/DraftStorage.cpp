...
#if JUCE_LINUX
    draftsDir = juce::File::getSpecialLocation(juce::File::userHomeDirectory)
                    .getChildFile(".local/share/Sidechain/drafts");
#elif JUCE_MAC
    draftsDir = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
                    .getChildFile("Sidechain/drafts");
#else // Windows
    draftsDir = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
                    .getChildFile("Sidechain/drafts");
#endif
...
bool DraftStorage::writeAudioFile(const juce::File& file, const juce::AudioBuffer<float>& buffer, double sampleRate)
{
    ...
    if (success)
    {
        Log::debug("DraftStorage::writeAudioFile: Wrote " + juce::String(buffer.getNumSamples()) +
                   " samples to " + file.getFileName());
    }
    else
    {
...
bool DraftStorage::readAudioFile(const juce::File& file, juce::AudioBuffer<float>& buffer, double& sampleRate)
{
    ...
    if (success)
    {
        Log::debug("DraftStorage::readAudioFile: Read " + juce::String(buffer.getNumSamples()) +
                   " samples from " + file.getFileName());
    }
    else
    {
...
Draft DraftStorage::saveDraft(const Draft& draft, const juce::AudioBuffer<float>& audioBuffer)
{
    ...
    Log::info("DraftStorage::saveDraft: Saved draft " + savedDraft.id +
              " (" + savedDraft.getFormattedDuration() + ")");
...
{
    return getMetadataFile(AUTO_RECOVERY_ID).existsAsFile() &&
           getAudioFile(AUTO_RECOVERY_ID).existsAsFile();
}
