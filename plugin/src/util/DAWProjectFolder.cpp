...
// Internal DAW detection (duplicated from NetworkClient to avoid circular
// dependency)
...
DAWProjectInfo detectDAWProjectFolder(const juce::String &detectedDAWName) {
  ...
  if (dawName.containsIgnoreCase("Ableton")) {
    ...
    // MIDI files typically go in: Project Folder / Samples / Imported / MIDI
    // Files
    ...
    // FL Studio: Project files are .flp, typically in user's FL Studio projects
    // folder
    juce::File currentDir = juce::File::getCurrentWorkingDirectory();
    ...
    if (flpFiles.size() > 0) {
      projectFolder = currentDir;
      midiFolder = projectFolder; // FL Studio projects can have MIDI files in
                                  // project folder
    } else {
      // Try FL Studio default locations
      juce::File userData =
          juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile("FL Studio");
    ...
    if (logicFiles.size() > 0) {
      projectFolder = currentDir;
      // Logic projects are packages - MIDI files typically in Audio Files or
      // MIDI Files
    ...
    if (rppFiles.size() > 0) {
      ...
      juce::File userData =
          juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile("REAPER Media");
  ...
  // For MIDI folder, use project folder if available, otherwise try detected
  // MIDI folder
...
juce::File getDefaultMIDIFolder() {
  juce::File midiFolder =
      juce::File::getSpecialLocation(juce::File::userDocumentsDirectory).getChildFile("Sidechain").getChildFile("MIDI");
