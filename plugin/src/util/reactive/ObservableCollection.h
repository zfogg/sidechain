...
/**
 * ObservableArray<T> - A thread-safe reactive array that notifies observers on
 * collection changes
 *
 * Wraps juce::Array<T> and provides notifications when items are added,
 * removed, or modified.
 *
 * Features:
...
public:
  ...
  /**
   * Begin batch update - suppress notifications until endBatchUpdate() is
   * called
   */
   ...
   */
  template <typename U> std::shared_ptr<ObservableArray<U>> map(std::function<U(const T &)> transform) {
    ...
    observeItemAdded([mapped, transform](int index, const T &item) { mapped->insert(index, transform(item)); });

    observeItemRemoved([mapped](int index, const T &) { mapped->remove(index); });
  ...
  std::shared_ptr<ObservableArray<T>> filter(std::function<bool(const T &)> predicate) {
    ...
    observeItemRemoved([filtered](int /* index */, const T &item) { filtered->removeItem(item); });
 ...
 * @tparam V Value type
 */
template <typename K, typename V> class ObservableMap {
public:
  using Key = K;
  using Value = V;
