...
/**
 * ObservableProperty<T> - A thread-safe reactive property that notifies
 ...
 * - Audio-thread safe: can read via get() without locks (atomic for small
 * types)
 *
 * Example:
...
public:
   ...
   */
  explicit ObservableProperty(const T &initialValue = T()) : value(initialValue) {}
   ...
   */
  template <typename U> std::shared_ptr<ObservableProperty<U>> map(std::function<U(const T &)> transform) {
    auto mapped = std::make_shared<ObservableProperty<U>>(transform(get()));

    // Subscribe to changes and update mapped property
    observe([mapped, transform](const T &newValue) { mapped->set(transform(newValue)); });

    return mapped;
  }

  /**
   * Create a filtered observable property that only notifies when predicate is
   * true
  ...
  std::shared_ptr<ObservableProperty<T>> debounce(int delayMs) {
    ...
    observe([debounced, pendingValue, pendingTimer, delayMs](const T &newValue) {
      ...
      *pendingTimer = std::make_unique<juce::Timer>();
      ...
      // For now, return immediately (debounce logic should be implemented
      // in UI components)
...
public:
  ...
  explicit AtomicObservableProperty(const T &initialValue = T()) : atomicValue(initialValue) {}
  ...
  /**
   * Set new value and notify observers (not audio-thread safe, use from UI
   * thread)
   */
  void set(const T &newValue) {
