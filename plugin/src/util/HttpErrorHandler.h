...
public:
  ...
  void reportError(const juce::String &endpoint, const juce::String &method, int statusCode,
                   const juce::String &errorMessage, const juce::String &responseBody = "") {
    HttpError error;
    ...
    juce::MessageManager::callAsync([error]() { showErrorPopup(error); });
#endif
...
private:
  ...
  static void showErrorPopup(const HttpError &error) {
    ...
    // Show alert dialog
    juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon, "HTTP Error (Debug)", message, "OK");
#else
