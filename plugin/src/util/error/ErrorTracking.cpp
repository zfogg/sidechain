...
void ErrorTracker::recordError(ErrorSource source, const juce::String &message, ErrorSeverity severity,
...
void ErrorTracker::recordException(ErrorSource source, const std::exception &exception,
                                   const std::map<juce::String, juce::String> &context) {
...
juce::String ErrorTracker::exportAsCsv() const {
  ...
  for (const auto &error : errors_) {
    ...
    csv << error.id << "," << ErrorInfo::sourceToString(error.source) << ","
        << ErrorInfo::severityToString(error.severity) << ","
        << "\"" << error.message.replace("\"", "\"\"") << "\"," << error.occurrenceCount << "," << oss.str() << "\n";
  }
...
std::optional<size_t> ErrorTracker::findDuplicateError(const ErrorInfo &error) const {
  for (size_t i = 0; i < errors_.size(); ++i) {
    if (errors_[i].hash == error.hash && errors_[i].message == error.message && errors_[i].source == error.source) {
      return i;
    }
  }
  return std::nullopt;
