...
template <typename T> struct CacheEntry {
  ...
  bool isExpired() const {
    ...
    auto age =
        std::chrono::duration_cast<std::chrono::seconds>(now - createdAt);
   ...
   */
  void updateAccessTime() { lastAccessedAt = std::chrono::steady_clock::now(); }
};
...
public:
   ...
   */
  MemoryCache(size_t maxSizeBytes = 100 * 1024 * 1024,
              size_t maxCountItems = 10000)
      : maxSizeBytes_(maxSizeBytes), maxCountItems_(maxCountItems),
        currentSizeBytes_(0) {}
   ...
   */
  void put(const K &key, const V &value, int ttlSeconds = 0,
           size_t sizeBytes = 0) {
    std::lock_guard<std::mutex> lock(mutex_);
...
private:
  ...
  void evictIfNeeded(size_t newEntrySize) {
    // Check size limit
    while (maxSizeBytes_ > 0 &&
           currentSizeBytes_ + newEntrySize > maxSizeBytes_) {
      if (cache_.empty())
        break;
...
private:
   ...
   */
  std::optional<CacheEntry<juce::MemoryBlock>>
  loadMetadata(const juce::File &file) const {
    ...
    if (metadataFile.loadFileAsData(metadata)) {
      ...
      juce::String metaStr(static_cast<const char *>(metadata.getData()),
                           metadata.getSize());
  ...
  size_t calculateDirectorySize(const juce::File &dir) const {
    size_t totalSize = 0;
    for (const auto &f :
         dir.findChildFiles(juce::File::findFilesAndDirectories, true)) {
      if (f.existsAsFile())
  ...
  void evictOldestIfNeeded() {
    ...
    if (currentSize > maxSizeBytes_) {
      ...
      for (const auto &f :
           cacheDir_.findChildFiles(juce::File::findFiles, false, "*.cache")) {
...
public:
   ...
   */
  MultiTierCache(size_t memoryMaxBytes,
                 [[maybe_unused]] const juce::File &diskDir,
   ...
   */
  void put(const K &key, const V &value, int ttlSeconds = 3600,
           bool promoteToMemory = true) {
    if (promoteToMemory) {
   ...
   * Remove from cache
   */
  bool remove(const K &key) { return memoryCache_.remove(key); }

  /**
   * Clear all caches
   */
  void clear() { memoryCache_.clear(); }
  ...
  CacheStats getStats() const {
    return {memoryCache_.getCurrentSizeBytes(), memoryCache_.getItemCount(),
            memoryCache_.getHitRate()};
  }
...
private:
  ...
  void statsHit() { hitCount_++; }

  void statsMiss() { missCount_++; }
};
