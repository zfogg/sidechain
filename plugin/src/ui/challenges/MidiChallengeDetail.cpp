...
void MidiChallengeDetail::mouseWheelMove(const juce::MouseEvent& /*event*/,
...
void MidiChallengeDetail::drawEntryCard(juce::Graphics& g, juce::Rectangle<int> bounds, const MIDIChallengeEntry& entry, int index)
{
    ...
    if (index < 3)
    {
        ...
        g.setColour(index == 0 ? juce::Colour(0xFFFFD700) : (index == 1 ? juce::Colour(0xFFC0C0C0) : juce::Colour(0xFFCD7F32)));
...
juce::String MidiChallengeDetail::formatConstraints(const MIDIChallengeConstraints& constraints) const
{
    ...
    {
        if (constraints.durationMin > 0 && constraints.durationMax > 0)
            parts.add("Duration: " + juce::String(constraints.durationMin, 1) + "-" + juce::String(constraints.durationMax, 1) + "s");
