 ...
 * - Slide: View slides in from direction, previous slides out in opposite
 * direction
...
private:
  ...
  void slideTransition(juce::Component *oldView, juce::Component *newView,
    ...
    auto slideNewView =
        TransitionAnimation<int>::create(-parentWidth * direction, 0, duration)
            ->withEasing(Easing::easeOutCubic)
    ...
    auto slideOldView =
        TransitionAnimation<int>::create(0, parentWidth * direction, duration)
            ->withEasing(Easing::easeOutCubic)
  ...
  void slideVerticalTransition(juce::Component *oldView,
    ...
    auto slideNewView =
        TransitionAnimation<int>::create(-parentHeight * direction, 0, duration)
            ->withEasing(Easing::easeOutCubic)
            ->onProgress([newView](int offset) {
              if (newView) {
                auto bounds = newView->getBounds();
    ...
    // Slide old view out
    auto slideOldView =
        TransitionAnimation<int>::create(0, parentHeight * direction, duration)
            ->withEasing(Easing::easeOutCubic)
            ->onProgress([oldView](int offset) {
              if (oldView) {
                auto bounds = oldView->getBounds();
