 ...
 *
 * Individual toast component that displays a message and can be clicked to
 * dismiss.
 */
...
public:
  ...
  void setMaxVisibleToasts(int max) {
    maxVisibleToasts = max;
  }

  /** Set position (from top or bottom of parent) */
  void setPosition(bool fromTop) {
    positionFromTop = fromTop;
    updateLayout();
  }
