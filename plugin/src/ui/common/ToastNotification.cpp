...
ToastManager::ToastManager() {
  setInterceptsMouseClicks(false,
                           true); // Pass through clicks except to children
...
void ToastManager::addToast(std::unique_ptr<ToastNotification> toast) {
  toast->onDismiss = [this](ToastNotification *t) { removeToast(t); };
...
void ToastManager::removeToast(ToastNotification *toast) {
  toasts.erase(std::remove_if(toasts.begin(), toasts.end(),
                              [toast](const std::unique_ptr<ToastNotification> &t) { return t.get() == toast; }),
               toasts.end());
  updateLayout();
}
