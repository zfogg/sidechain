...
void ErrorState::paint(juce::Graphics &g) {
  ...
  if (compactMode) {
    ...
    int contentHeight = 0;
    if (showIcon)
      contentHeight += iconSize + spacing;
    contentHeight += 24; // Title
    contentHeight += spacing / 2;
    contentHeight += 40; // Message (approx 2 lines)
    if (onPrimaryAction)
      contentHeight += spacing + buttonHeight;
    if (onSecondaryAction)
      contentHeight += spacing / 2 + buttonHeight;
...
ErrorState::ErrorType ErrorState::detectErrorType(const juce::String &errorMessage) {
  ...
  if (errorMessage.containsIgnoreCase("timeout") || errorMessage.containsIgnoreCase("timed out")) {
    return ErrorType::Timeout;
  }

  // Offline - check before general network
  if (errorMessage.containsIgnoreCase("offline") || errorMessage.containsIgnoreCase("no internet")) {
    return ErrorType::Offline;
  }

  // Rate limiting
  if (errorMessage.containsIgnoreCase("429") || errorMessage.containsIgnoreCase("rate limit") ||
      errorMessage.containsIgnoreCase("too many requests") || errorMessage.containsIgnoreCase("slow down")) {
    return ErrorType::RateLimit;
  }

  // Network/connection errors
  if (errorMessage.containsIgnoreCase("network") || errorMessage.containsIgnoreCase("connection") ||
      errorMessage.containsIgnoreCase("connect") || errorMessage.containsIgnoreCase("unreachable")) {
    return ErrorType::Network;
  }

  // Authentication errors
  if (errorMessage.containsIgnoreCase("auth") || errorMessage.containsIgnoreCase("unauthorized") ||
      errorMessage.containsIgnoreCase("401") || errorMessage.containsIgnoreCase("forbidden") ||
      errorMessage.containsIgnoreCase("403") || errorMessage.containsIgnoreCase("not authenticated") ||
      errorMessage.containsIgnoreCase("sign in") || errorMessage.containsIgnoreCase("login")) {
    return ErrorType::Auth;
  }

  // Permission errors
  if (errorMessage.containsIgnoreCase("permission") || errorMessage.containsIgnoreCase("denied") ||
      errorMessage.containsIgnoreCase("not allowed") || errorMessage.containsIgnoreCase("microphone") ||
      errorMessage.containsIgnoreCase("access")) {
    return ErrorType::Permission;
  }

  // Not found
  if (errorMessage.containsIgnoreCase("not found") || errorMessage.containsIgnoreCase("404") ||
      errorMessage.containsIgnoreCase("doesn't exist") || errorMessage.containsIgnoreCase("does not exist")) {
    return ErrorType::NotFound;
  }

  // Parsing/data errors
  if (errorMessage.containsIgnoreCase("parse") || errorMessage.containsIgnoreCase("json") ||
      errorMessage.containsIgnoreCase("invalid format") || errorMessage.containsIgnoreCase("unexpected") ||
      errorMessage.containsIgnoreCase("malformed")) {
    return ErrorType::Parsing;
  }

  // Validation errors
  if (errorMessage.containsIgnoreCase("validation") || errorMessage.containsIgnoreCase("invalid") ||
      errorMessage.containsIgnoreCase("required") || errorMessage.containsIgnoreCase("must be") ||
      errorMessage.containsIgnoreCase("cannot be empty")) {
    return ErrorType::Validation;
  }

  // Upload errors
  if (errorMessage.containsIgnoreCase("upload") || errorMessage.containsIgnoreCase("file size") ||
      errorMessage.containsIgnoreCase("too large")) {
    return ErrorType::Upload;
  }

  // Audio errors
  if (errorMessage.containsIgnoreCase("audio") || errorMessage.containsIgnoreCase("playback") ||
      errorMessage.containsIgnoreCase("codec") || errorMessage.containsIgnoreCase("format not supported")) {
    return ErrorType::Audio;
  }

  // Server errors (check after more specific types)
  if (errorMessage.containsIgnoreCase("500") || errorMessage.containsIgnoreCase("502") ||
      errorMessage.containsIgnoreCase("503") || errorMessage.containsIgnoreCase("504") ||
      errorMessage.containsIgnoreCase("server error") || errorMessage.containsIgnoreCase("internal error") ||
...
juce::String ErrorState::getDefaultMessageForType() const {
  ...
  case ErrorType::RateLimit:
    return "You're making too many requests. Please wait a moment and try "
           "again.";
  ...
  // Media errors
  case ErrorType::Upload:
    return "Failed to upload your file. Please check your connection and try "
           "again.";
  case ErrorType::Audio:
...
std::unique_ptr<ErrorState> ErrorState::permissionError(const juce::String &permissionName,
