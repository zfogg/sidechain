...
public:
    ...
    /** Configure this error state from an error message (detects type and sets message) */
    ...
    /** Show/hide the icon */
    void setShowIcon(bool show) { showIcon = show; repaint(); }

    /** Set compact mode (smaller layout for inline use) */
    void setCompact(bool compact) { compactMode = compact; repaint(); }
    ...
    static std::unique_ptr<ErrorState> emptyState(const juce::String& title, const juce::String& message,
                                                   const juce::String& actionText = "", std::function<void()> onAction = nullptr);
    ...
    static std::unique_ptr<ErrorState> permissionError(const juce::String& permissionName, std::function<void()> onSettings = nullptr);
