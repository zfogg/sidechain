...
void SkeletonLoader::drawLines(juce::Graphics &g, juce::Rectangle<int> bounds, int lineCount, int lineHeight,
                               int lineSpacing) {
  int y = bounds.getY();
...
void ProfileSkeleton::drawSkeletonShapes(juce::Graphics &g) {
  ...
  auto avatarSize = 80;
  auto avatarBounds = juce::Rectangle<int>(bounds.getCentreX() - avatarSize / 2, bounds.getY(), avatarSize, avatarSize);
  drawCircle(g, avatarBounds);
...
void StoryCircleSkeleton::drawSkeletonShapes(juce::Graphics &g) {
  ...
  if (bounds.getHeight() > 70) {
    auto labelBounds = juce::Rectangle<int>(bounds.getX() - 10, bounds.getBottom() + 4, bounds.getWidth() + 20, 12);
