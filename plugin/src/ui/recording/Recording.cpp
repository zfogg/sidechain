...
void Recording::timerCallback() {
  ...
  if (currentState == State::Recording && !audioProcessor.isRecording()) {
    ...
    Log::info("Recording::timerCallback: Recording stopped externally (likely "
              "max length reached)");
    stopRecording();
  }
...
void Recording::startRecording() {
  ...
  if (ProgressiveKeyDetector::isAvailable()) {
    if (progressiveKeyDetector.start(sampleRate)) {
      ...
      Log::warn("Recording::startRecording: Failed to start progressive key "
                "detection");
    }
  }

  Log::debug("Recording::startRecording: State changed to Recording, animation "
             "started");
  repaint();
}
