...
void DraftsView::paint(juce::Graphics& g)
{
    ...
    else
    {
        ...
        for (int i = 0; i < drafts.size(); ++i)
        {
            ...
            if (cardBounds.getBottom() > contentBounds.getY() &&
                cardBounds.getY() < contentBounds.getBottom())
            {
...
void DraftsView::drawHeader(juce::Graphics& g)
{
    ...
    g.setFont(20.0f);
    g.drawText(juce::String::fromUTF8("\xE2\x86\x90"), backBounds, juce::Justification::centred); // <-
    ...
    if (!drafts.isEmpty())
    {
        g.setColour(SidechainColors::textSecondary());
        g.setFont(14.0f);
        g.drawText(juce::String(drafts.size()) + " draft" + (drafts.size() != 1 ? "s" : ""),
                   bounds.withTrimmedRight(60), juce::Justification::centredRight);
    }
...
void DraftsView::drawDraftCard(juce::Graphics& g, const Draft& draft, juce::Rectangle<int> bounds, int index)
{
    ...
    g.setFont(14.0f);
    g.drawText(juce::String::fromUTF8("\xF0\x9F\x97\x91"), deleteBounds, juce::Justification::centred); // Trash icon
}
...
void DraftsView::drawEmptyState(juce::Graphics& g)
{
    ...
    g.drawText(juce::String::fromUTF8("\xF0\x9F\x93\x9D"), // Memo icon
               bounds.withHeight(60).withY(bounds.getCentreY() - 60),
               juce::Justification::centred);

    g.setFont(16.0f);
    g.drawText("No drafts yet",
               bounds.withHeight(24).withY(bounds.getCentreY() + 10),
               juce::Justification::centred);

    g.setFont(13.0f);
    g.drawText("Save recordings as drafts to continue later",
               bounds.withHeight(20).withY(bounds.getCentreY() + 38),
               juce::Justification::centred);
}
...
void DraftsView::drawDeleteConfirmation(juce::Graphics& g)
{
    ...
    g.setColour(SidechainColors::textSecondary());
    g.setFont(13.0f);
    g.drawText("This action cannot be undone.",
               dialogBounds.withTrimmedTop(50).withHeight(40),
               juce::Justification::centred);
...
{
    return juce::Rectangle<int>(PADDING, HEADER_HEIGHT + PADDING,
                                 getWidth() - PADDING * 2 - 10, RECOVERY_BANNER_HEIGHT);
}
