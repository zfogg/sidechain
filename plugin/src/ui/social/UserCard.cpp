...
void UserCard::setIsFollowing(bool following) {
  if (user.isFollowing != following) {
    ...
    Log::debug("UserCard: Follow state changed - user: " + user.id +
               ", following: " + juce::String(following ? "true" : "false"));
    repaint();
  }
}
...
void UserCard::drawGenreBadge(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  UIHelpers::drawPillBadge(g, bounds.getX(), bounds.getY() + 1, user.genre, Colors::badge, Colors::textSecondary, 10.0f,
                           6, 4);
}
