...
void StoryViewer::mouseUp(const juce::MouseEvent &event) {
  ...
  if (story && story->userId == currentUserId) {
    ...
    if (shareButtonArea.contains(pos)) {
      ...
      shareMenu.showMenuAsync(juce::PopupMenu::Options(), [this, storyId, currentStory](int result) {
        if (result == 1) {
...
void StoryViewer::drawHeader(juce::Graphics &g) {
  ...
  if (story->filename.isNotEmpty()) {
    ...
    g.setFont(12.0f);
    g.drawText("by " + (story->username.isNotEmpty() ? story->username : "Unknown"), bounds.removeFromTop(16),
               juce::Justification::centredLeft);
  } else {
    // Fallback: show username as main header
    g.drawText(story->username.isNotEmpty() ? story->username : "Unknown", bounds.removeFromTop(20),
               juce::Justification::centredLeft);
  }
...
void StoryViewer::drawViewCount(juce::Graphics &g) {
  ...
  juce::String viewText = juce::String(story->viewCount) + " view" + (story->viewCount != 1 ? "s" : "");
...
void StoryViewer::drawPlayPauseOverlay(juce::Graphics &g) {
  ...
  juce::Path playIcon;
  playIcon.addTriangle(center.x - iconSize * 0.3f, center.y - iconSize * 0.4f, center.x - iconSize * 0.3f,
                       center.y + iconSize * 0.4f, center.x + iconSize * 0.4f, center.y);
...
void StoryViewer::drawExpiredMessage(juce::Graphics &g) {
  ...
  g.drawText("This story has expired and is no longer available.",
             juce::Rectangle<int>(0, static_cast<int>(center.y), getWidth(), 20), juce::Justification::centred);
}
...
void StoryViewer::handleDeleteStory(const juce::String &storyId) {
  ...
  auto options = juce::MessageBoxOptions()
                     ...
                     .withMessage("Are you sure you want to delete this story? "
  ...
  networkClient->downloadMIDI(
      story.midiPatternId, targetFile, [targetFile, notificationMessage](Outcome<juce::var> result) {
        if (result.isOk()) {
          ...
          juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon, "Download Failed",
                                    ...
                                    [targetFile, notificationMessage](Outcome<juce::var> result) {
                                      if (result.isOk()) {
                                        ...
                                        juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::InfoIcon,
                                                                               "Audio Downloaded", notificationMessage);
                                      } else {
                                        Log::error("Failed to download story audio: " + result.getError());
                                        juce::AlertWindow::showMessageBoxAsync(
                                            juce::MessageBoxIconType::WarningIcon, "Download Failed",
...
void StoryViewer::loadCurrentStory() {
  ...
  if (story->audioUrl.isNotEmpty() && audioPlayer) {
    ...
    auto waveformBounds = contentArea.reduced(20).removeFromTop(100);
    WaveformGenerator::generateWaveformPathFromUrl(story->audioUrl, waveformBounds.reduced(10),
                                                   [this](juce::Path path) {
