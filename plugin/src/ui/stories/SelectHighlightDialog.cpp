...
void SelectHighlightDialog::drawHeader(juce::Graphics& g)
{
    ...
    g.drawText("Select a highlight or create a new one",
               headerBounds.reduced(PADDING, 0).translated(0, 24),
...
void SelectHighlightDialog::drawHighlightsList(juce::Graphics& g)
{
    ...
    auto createNewBounds = juce::Rectangle<int>(
        contentBounds.getX(), yOffset,
    ...
    for (int i = 0; i < highlights.size(); ++i)
    {
        auto itemBounds = juce::Rectangle<int>(
            contentBounds.getX(), yOffset + i * ITEM_HEIGHT,
...
void SelectHighlightDialog::drawHighlightItem(juce::Graphics& g, const StoryHighlight& highlight, juce::Rectangle<int> bounds)
{
    // Background
    ...
    else
    {
        ...
        juce::String initial = highlight.name.isNotEmpty()
            ? highlight.name.substring(0, 1).toUpperCase()
            : "?";
    ...
    juce::String countText = juce::String(highlight.storyCount) + " " +
                             (highlight.storyCount == 1 ? "story" : "stories");
...
void SelectHighlightDialog::drawEmptyState(juce::Graphics& g)
{
    ...
    auto createNewBounds = juce::Rectangle<int>(
        contentBounds.getX(), contentBounds.getY() + 10,
        contentBounds.getWidth() - 12, CREATE_NEW_HEIGHT
    );
    drawCreateNewItem(g, createNewBounds);
    ...
    g.setColour(SidechainColors::textSecondary());
    g.setFont(juce::Font(juce::FontOptions().withHeight(14.0f)));
    g.drawText("No highlights yet.\nCreate one to save your stories!",
               messageBounds, juce::Justification::centredTop);
}
