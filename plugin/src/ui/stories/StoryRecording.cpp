...
void StoryRecording::resized() {
  ...
  if (currentState == State::Preview) {
    // Preview layout: waveform at top, piano roll in middle, playback controls,
    // metadata, action buttons at bottom
    ...
    // Recording layout: countdown ring + record button centered with text
    // anchored to it
  ...
  if (currentState == State::Preview) {
    ...
    if (sourceToggleArea.contains(pos)) {
      ...
      currentRecordingSource =
          (currentRecordingSource == RecordingSource::DAW) ? RecordingSource::Microphone : RecordingSource::DAW;
...
void StoryRecording::drawRecordingState(juce::Graphics &g) {
  ...
  if (canStopRecording()) {
    ...
    g.drawText("Min " + juce::String(static_cast<int>(MIN_DURATION_SECONDS)) + "s required",
               cancelButtonArea.withY(cancelButtonArea.getY() - 30).withHeight(20), juce::Justification::centred);
  }
...
void StoryRecording::drawHeader(juce::Graphics &g) {
  ...
  if (currentState == State::Idle) {
    ...
    g.drawText("5-60 seconds - Expires in 24 hours",
               headerArea.withY(headerArea.getY() + 35).withHeight(25).reduced(20, 0), juce::Justification::centred);
  }
}
...
void StoryRecording::drawRecordButton(juce::Graphics &g) {
  ...
  if (currentState == State::Recording) {
    ...
    g.fillRoundedRectangle(center.x - squareSize, center.y - squareSize, squareSize * 2, squareSize * 2, 8.0f);

    // Outer ring
    g.setColour(StoryColors::recordRed.withAlpha(0.3f));
    g.drawEllipse(center.x - adjustedRadius, center.y - adjustedRadius, adjustedRadius * 2, adjustedRadius * 2, 3.0f);
  } else {
    // Record circle
    g.setColour(StoryColors::recordRed);
    ...
    g.setColour(StoryColors::textPrimary);
    g.fillEllipse(center.x - centerRadius, center.y - centerRadius, centerRadius * 2, centerRadius * 2);
  }
}
...
void StoryRecording::drawTimeDisplay(juce::Graphics &g) {
  ...
  g.drawText("/ " + formatTime(MAX_DURATION_SECONDS),
             timeDisplayArea.withY(timeDisplayArea.getBottom() - 5).withHeight(25), juce::Justification::centred);
}
...
void StoryRecording::drawCountdownRing(juce::Graphics &g) {
  ...
  if (currentState == State::Recording) {
    ...
    arc.addArc(center.x - radius, center.y - radius, radius * 2, radius * 2, startAngle, endAngle, true);
...
void StoryRecording::drawPlaybackControls(juce::Graphics &g) {
  ...
  if (isPreviewPlaying) {
    ...
    // Play icon (triangle)
    float size = 10.0f;
    iconPath.addTriangle(center.x - size * 0.3f, center.y - size * 0.4f, center.x - size * 0.3f, center.y + size * 0.4f,
...
void StoryRecording::confirmRecording() {
  ...
  if (onRecordingComplete && recordedAudio.getNumSamples() > 0) {
    ...
    Log::info("StoryRecording::confirmRecording: Calling onRecordingComplete "
              "callback");
