...
void CircularVisualization::drawBackground(juce::Graphics& g)
{
    ...
    juce::ColourGradient gradient(
        CircularColors::background.brighter(0.1f), center,
...
void CircularVisualization::drawActiveNotes(juce::Graphics& g)
{
    for (const auto& note : notes)
    {
        ...
        juce::ColourGradient glowGradient(
            noteColor.withAlpha(0.8f * pulse), pos,
            noteColor.withAlpha(0.0f), pos.translated(glowSize, 0),
            true);
...
void CircularVisualization::drawCenterInfo(juce::Graphics& g)
{
    ...
    auto centerBounds = juce::Rectangle<float>(
        center.x - centerSize, center.y - centerSize / 2,
        centerSize * 2, centerSize);
