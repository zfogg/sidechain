...
void NoteWaterfall::setMIDIData(const juce::var& midiData)
{
    ...
    Log::info("NoteWaterfall: Loaded " + juce::String(notes.size()) + " notes, " +
...
void NoteWaterfall::drawBackground(juce::Graphics& g)
{
    ...
    juce::ColourGradient gradient(
        WaterfallColors::background.darker(0.5f), 0, 0,
        WaterfallColors::background, 0, static_cast<float>(bounds.getHeight()),
        false);
...
void NoteWaterfall::drawNotes(juce::Graphics& g)
{
    ...
    for (const auto& note : notes)
    {
        ...
        auto noteBounds = juce::Rectangle<float>(
            x + 2, topY, noteWidth - 4, noteHeight);
...
void NoteWaterfall::drawActiveNotesGlow(juce::Graphics& g)
{
    ...
    for (const auto& note : notes)
    {
        ...
        juce::ColourGradient glowGradient(
            noteColor.withAlpha(0.6f * pulse), glowCenter,
            noteColor.withAlpha(0.0f), glowCenter.translated(0, -glowRadius),
            true);
        g.setGradientFill(glowGradient);

        auto glowBounds = juce::Rectangle<float>(
            x - 5, catchLineY - glowRadius, noteWidth + 10, glowRadius);
