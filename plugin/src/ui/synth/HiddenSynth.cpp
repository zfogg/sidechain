...
void HiddenSynth::drawUnlockAnimation(juce::Graphics &g) {
  ...
  // Background gradient
  g.setGradientFill(juce::ColourGradient(juce::Colour(0xff0f0f1a), bounds.getCentre(), juce::Colour(0xff2a1a4a),
...
void HiddenSynth::sliderValueChanged(juce::Slider *slider) {
  if (slider == &attackSlider || slider == &decaySlider || slider == &sustainSlider || slider == &releaseSlider) {
    synthEngine.setADSR(static_cast<float>(attackSlider.getValue()), static_cast<float>(decaySlider.getValue()),
