...
void TwoFactorSettings::showModal(juce::Component* parentComponent)
{
    if (parentComponent)
    {
        ...
        setBounds(bounds.getCentreX() - width / 2,
                  bounds.getCentreY() - height / 2,
                  width, height);
...
void TwoFactorSettings::paint(juce::Graphics& g)
{
    ...
    switch (currentState)
    {
        ...
        case State::SetupType:
            ...
            g.drawText("Choose Authentication Method", contentBounds.removeFromTop(40),
        ...
        case State::SetupPassword:
            g.setColour(Colors::textPrimary);
            g.setFont(juce::Font(18.0f).boldened());
            g.drawText("Enter Your Password", contentBounds.removeFromTop(40),
                      juce::Justification::centred);
            g.setColour(Colors::textSecondary);
            g.setFont(14.0f);
            g.drawText("Required to enable two-factor authentication",
                      contentBounds.removeFromTop(30), juce::Justification::centred);
            break;
        ...
        case State::SetupVerify:
            g.setColour(Colors::textPrimary);
            g.setFont(juce::Font(18.0f).boldened());
            g.drawText("Verify Setup", contentBounds.removeFromTop(40),
                      juce::Justification::centred);
            g.setColour(Colors::textSecondary);
            g.setFont(14.0f);
            g.drawText("Enter the code from your authenticator app",
                      contentBounds.removeFromTop(30), juce::Justification::centred);
            break;
        ...
        case State::Disabling:
            g.setColour(Colors::textPrimary);
            g.setFont(juce::Font(18.0f).boldened());
            g.drawText("Disable Two-Factor Authentication", contentBounds.removeFromTop(40),
                      juce::Justification::centred);
            g.setColour(Colors::textSecondary);
            g.setFont(14.0f);
            g.drawText("Enter a 2FA code or your password",
                      contentBounds.removeFromTop(30), juce::Justification::centred);
            break;
    }
    ...
    if (statusMessage.isNotEmpty())
    {
        ...
        g.setFont(14.0f);
        g.drawText(statusMessage, getLocalBounds().removeFromBottom(40).reduced(PADDING, 0),
                  juce::Justification::centred);
    }
}
...
void TwoFactorSettings::drawStatus(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    g.setFont(juce::Font(18.0f).boldened());
    g.drawText(statusText, statusArea.withTrimmedLeft(PADDING + 20).removeFromTop(30),
    ...
    if (twoFactorEnabled)
    {
        ...
        g.drawText("Method: " + typeLabel,
                  statusArea.withTrimmedLeft(PADDING + 20).withTrimmedTop(30).removeFromTop(25),
                  juce::Justification::centredLeft);
    ...
    else
    {
        ...
        g.drawText("Add an extra layer of security to your account",
                  statusArea.withTrimmedLeft(PADDING + 20).withTrimmedTop(30),
                  juce::Justification::centredLeft);
    }
}
...
void TwoFactorSettings::drawSetupInfo(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    if (setupType == "hotp")
    {
        g.drawText("Configure Your YubiKey", bounds.removeFromTop(30),
                  juce::Justification::centredLeft);
    }
    else
    {
        g.drawText("Scan QR Code or Enter Secret", bounds.removeFromTop(30),
                  juce::Justification::centredLeft);
    }
    ...
    if (setupType != "hotp" && setupQRUrl.isNotEmpty())
    {
        g.setColour(Colors::textSecondary);
        g.setFont(14.0f);
        g.drawText("Scan this URL as QR code in your authenticator app:",
                  bounds.removeFromTop(25), juce::Justification::centredLeft);
    ...
    g.setColour(Colors::textSecondary);
    g.setFont(14.0f);
    g.drawText("Or enter this secret manually:", bounds.removeFromTop(25),
              juce::Justification::centredLeft);
...
void TwoFactorSettings::drawBackupCodes(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    g.drawText("Save Your Backup Codes!", bounds.removeFromTop(25),
              juce::Justification::centredLeft);

    g.setColour(Colors::textSecondary);
    g.setFont(12.0f);
    g.drawText("Use these if you lose access to your authenticator",
    ...
    for (int i = 0; i < backupCodes.size(); ++i)
    {
        ...
        auto codeBounds = juce::Rectangle<int>(
            codesBounds.getX() + col * codeWidth + 10,
            codesBounds.getY() + row * codeHeight + 8,
            codeWidth - 20, codeHeight
        );
