...
juce::String UserProfile::getAvatarUrl() const {
  // Backend returns avatar_url as the effective URL (prefers
  // profile_picture_url over oauth) Keep this logic for backwards compatibility
...
void Profile::drawHeader(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  g.setGradientFill(juce::ColourGradient(Colors::headerBg, 0.0f, 0.0f, Colors::background, 0.0f,
  ...
  auto userInfoBounds = juce::Rectangle<int>(avatarBounds.getRight() + PADDING, avatarBounds.getY(),
                                             bounds.getWidth() - avatarBounds.getRight() - PADDING * 3, AVATAR_SIZE);
...
void Profile::drawAvatar(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  if (profile.isOnline || profile.isInStudio) {
    ...
    auto indicatorBounds = juce::Rectangle<int>(bounds.getRight() - indicatorSize + 3,
                                                bounds.getBottom() - indicatorSize + 3, indicatorSize, indicatorSize)
                               .toFloat();
...
void Profile::drawUserInfo(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  if (profile.isPrivate) {
    ...
    g.drawText(juce::String::fromUTF8("\xF0\x9F\x94\x92"), // 🔒 Lock emoji
               bounds.getX() + nameWidth + 8, bounds.getY() + 12, 24, 24, juce::Justification::centredLeft);
  } else {
    g.drawText(name, bounds.getX(), bounds.getY() + 10, bounds.getWidth(), 28, juce::Justification::centredLeft);
  }
  ...
  if (infoLine.isNotEmpty()) {
    g.setColour(Colors::textSecondary);
    g.setFont(12.0f);
    g.drawText(infoLine, bounds.getX(), bounds.getY() + 62, bounds.getWidth(), 18, juce::Justification::centredLeft);
  }
...
void Profile::drawStats(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  auto followersBounds =
      juce::Rectangle<int>(bounds.getX() + statSpacing, bounds.getY(), statSpacing, bounds.getHeight());
  g.setColour(Colors::textPrimary);
  g.setFont(juce::FontOptions(18.0f).withStyle("Bold"));
  g.drawText(StringFormatter::formatCount(profile.followerCount), followersBounds.withHeight(22),
             juce::Justification::centred);
  g.setColour(Colors::textSecondary);
  g.setFont(12.0f);
  g.drawText("Followers", followersBounds.withY(followersBounds.getY() + 22).withHeight(20),
             juce::Justification::centred);

  // Following count (tappable)
  auto followingBounds =
      juce::Rectangle<int>(bounds.getX() + statSpacing * 2, bounds.getY(), statSpacing, bounds.getHeight());
  g.setColour(Colors::textPrimary);
  g.setFont(juce::FontOptions(18.0f).withStyle("Bold"));
  g.drawText(StringFormatter::formatCount(profile.followingCount), followingBounds.withHeight(22),
             juce::Justification::centred);
  g.setColour(Colors::textSecondary);
  g.setFont(12.0f);
  g.drawText("Following", followingBounds.withY(followingBounds.getY() + 22).withHeight(20),
             juce::Justification::centred);
}
...
void Profile::drawBio(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  juce::GlyphArrangement glyphs;
  glyphs.addFittedText(g.getCurrentFont(), profile.bio, static_cast<float>(bounds.getX()),
                       static_cast<float>(bounds.getY()), static_cast<float>(bounds.getWidth()),
                       static_cast<float>(bounds.getHeight()), juce::Justification::topLeft, 3);
  glyphs.draw(g);
}
...
void Profile::drawGenreTags(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  for (const auto &genre : genres) {
    ...
    auto tagBounds = juce::Rectangle<int>(x, bounds.getY() + (bounds.getHeight() - tagHeight) / 2, tagWidth, tagHeight);
    g.setColour(Colors::badge);
...
void Profile::drawErrorState(juce::Graphics &g) {
  ...
  g.setColour(Colors::textSecondary);
  g.setFont(13.0f);
  g.drawText(errorMessage, bounds.withHeight(20).withY(bounds.getCentreY()), juce::Justification::centred);

  // Retry button
  auto retryBounds =
      bounds.withSize(120, 36).withCentre(juce::Point<int>(bounds.getCentreX(), bounds.getCentreY() + 40));
...
void Profile::mouseUp(const juce::MouseEvent &event) {
  ...
  if (profile.isOwnProfile(currentUserId)) {
    ...
    if (getNotificationSettingsButtonBounds().contains(pos)) {
      ...
      if (onNotificationSettingsClicked)
        ...
        Log::warn("Profile::mouseUp: Notification settings clicked but "
                  "callback not set");
      return;
    }

    // Note: Activity Status, Muted Users, and Two-Factor Auth are now in Edit
    // Profile / Settings They are no longer shown on the main profile page
  } else {
    ...
    if (getMuteButtonBounds().contains(pos)) {
      Log::info("Profile::mouseUp: Mute button clicked - userId: " + profile.id +
                ", currentlyMuted: " + juce::String(profile.isMuted ? "true" : "false"));
      handleMuteToggle();
      return;
    }
  }
  ...
  if (hasError) {
    auto retryBounds =
        getLocalBounds().withSize(120, 36).withCentre(juce::Point<int>(getWidth() / 2, getHeight() / 2 + 40));
...
void Profile::handleFollowToggle() {
  ...
  Log::info("Profile::handleFollowToggle: Toggling follow for userId: " + profile.id +
...
void Profile::handleMuteToggle() {
  ...
  Log::info("Profile::handleMuteToggle: Toggling mute for userId: " + profile.id + " - wasMuted: " +
            juce::String(wasMuted ? "true" : "false") + ", willMute: " + juce::String(willMute ? "true" : "false"));
...
void Profile::updatePostCards() {
  ...
  while (postCards.size() < userPosts.size()) {
    ...
    card->onUserClicked = [this](const Sidechain::FeedPost &post) {
      ...
      if (post.userId == profile.id) {
        Log::debug("Profile::updatePostCards: User clicked on own profile - no "
                   "action needed");
        return;
      }

      Log::debug("Profile::updatePostCards: User clicked on post card - "
                 "navigating to user: " +
                 post.userId);
  ...
  for (int i = 0; i < userPosts.size(); ++i) {
    ...
    Log::debug("Profile::updatePostCards: Setting post " + juce::String(i) + " - isFollowing: " +
               juce::String(userPosts[i].isFollowing ? "true" : "false") + ", userId: " + userPosts[i].userId);
  ...
  Log::debug("Profile::updatePostCards: Updated " + juce::String(userPosts.size()) + " post cards, " +
             juce::String(visibleCount) + " visible");
}
...
void Profile::showFollowersList(const juce::String &userId, FollowersList::ListType type) {
  if (followersListPanel == nullptr || userId.isEmpty()) {
    Log::warn("Profile::showFollowersList: Cannot show list - panel: " +
              juce::String(followersListPanel != nullptr ? "valid" : "null") + ", userId: " + userId);
    return;
  }
  ...
  // Set up the panel
  // FollowersList uses AppStore for state management and data loading
...
void Profile::queryPresenceForProfile() {
  if (!streamChatClient || profile.id.isEmpty()) {
    Log::debug("Profile::queryPresenceForProfile: Skipping - streamChatClient "
               "is null or profile ID is empty");
    return;
  }
