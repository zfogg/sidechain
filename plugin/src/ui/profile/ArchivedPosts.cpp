...
namespace
{
    namespace Colors
    {
        inline juce::Colour background()      { return SidechainColors::background(); }
        inline juce::Colour headerBg()        { return SidechainColors::backgroundLight(); }
        ...
        inline juce::Colour border()          { return SidechainColors::border(); }
        inline juce::Colour error()           { return SidechainColors::error(); }
    }
}
...
void ArchivedPosts::fetchArchivedPosts()
{
    ...
    networkClient->getArchivedPosts(PAGE_SIZE, currentOffset, [this](Outcome<juce::var> result) {
        ...
        Log::debug("ArchivedPosts: Loaded " + juce::String(archivedPosts.size()) + " archived posts, hasMore: " + (hasMore ? "true" : "false"));
...
void ArchivedPosts::updatePostCardPositions()
{
    ...
    for (auto* card : postCards)
    {
        card->setBounds(contentBounds.getX() + PADDING,
                       y,
                       contentBounds.getWidth() - PADDING * 2,
                       POST_CARD_HEIGHT);
...
void ArchivedPosts::setupPostCardCallbacks(PostCard* card)
{
    ...
    card->onLikeToggled = [this](const FeedPost& post, bool liked) {
        if (networkClient == nullptr) return;
