...
void Auth::paint(juce::Graphics &g) {
  ...
  if (currentMode == AuthMode::OAuthWaiting) {
    ...
    g.drawText("A browser window has been opened for " + oauthWaitingProvider + " sign-in.", browserMsgArea,
...
void Auth::drawLogo(juce::Graphics &g, juce::Rectangle<int> bounds) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    g.fillRoundedRectangle(static_cast<float>(barX), static_cast<float>(centerY - barHeight / 2),
                           static_cast<float>(barWidth), static_cast<float>(barHeight), 3.0f);
  }
...
void Auth::resized() {
  ...
  case AuthMode::Login: {
    ...
    loginForgotPasswordLink->setBounds(
        forgotPasswordBounds.withX(forgotPasswordBounds.getRight() - loginForgotPasswordLink->getWidth())
            .withWidth(loginForgotPasswordLink->getWidth()));
...
void Auth::textEditorReturnKeyPressed(juce::TextEditor &editor) {
  if (currentMode == AuthMode::Login) {
    ...
    if (&editor == signupEmailEditor.get()) {
      ...
      Log::debug("Auth: Return key pressed in signup username field, moving to "
                 "display name");
      signupDisplayNameEditor->grabKeyboardFocus();
    } else if (&editor == signupDisplayNameEditor.get()) {
      Log::debug("Auth: Return key pressed in signup display name field, "
                 "moving to password");
      signupPasswordEditor->grabKeyboardFocus();
    } else if (&editor == signupPasswordEditor.get()) {
      Log::debug("Auth: Return key pressed in signup password field, moving to "
                 "confirm password");
      signupConfirmPasswordEditor->grabKeyboardFocus();
    } else if (&editor == signupConfirmPasswordEditor.get()) {
      Log::debug("Auth: Return key pressed in signup confirm password field, "
                 "submitting");
      handleSignup();
    }
...
void Auth::handleLogin() {
  ...
  auto validator =
      InputValidator::create()->addRule("email", InputValidator::email())->addRule("password", passwordRule);
