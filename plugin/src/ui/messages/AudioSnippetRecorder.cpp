...
void AudioSnippetRecorder::timerCallback() {
  ...
  if (currentState == State::Recording && !audioProcessor.isRecording()) {
    Log::info("AudioSnippetRecorder::timerCallback: Recording stopped "
              "externally (likely max length reached)");
    stopRecording();
  }

  // Auto-stop if max duration reached
  if (currentState == State::Recording && hasReachedMaxDuration()) {
    Log::info("AudioSnippetRecorder::timerCallback: Max duration reached, "
              "auto-stopping");
    stopRecording();
  }
...
void AudioSnippetRecorder::startRecording() {
  ...
  // IMPORTANT GOTCHAS:
  // - Cannot start if processor is already recording (e.g., for a full post
  // upload)
  ...
  if (audioProcessor.isRecording()) {
    Log::warn("AudioSnippetRecorder::startRecording: Processor already "
              "recording, cannot start snippet");
    juce::MessageManager::callAsync([]() {
      juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon, "Recording Busy",
                                             "Cannot start audio snippet recording. The audio processor is "
                                             "already recording.");
    });
    return;
  }
...
void AudioSnippetRecorder::stopRecording() {
  ...
  // - Audio buffer is copied from processor (processor may continue recording
  // for other purposes)
  ...
  if (recordedAudio.getNumSamples() > 0) {
    ...
    Log::info("AudioSnippetRecorder::stopRecording: Recording complete, "
              "showing preview");
  } else {
    currentState = State::Idle;
    Log::warn("AudioSnippetRecorder::stopRecording: Recording stopped but no "
              "audio captured");
  }
...
void AudioSnippetRecorder::sendRecording() {
  ...
  if (onRecordingComplete && recordedAudio.getNumSamples() > 0) {
    ...
    Log::warn("AudioSnippetRecorder::sendRecording: No audio to send or "
              "callback not set");
  }
