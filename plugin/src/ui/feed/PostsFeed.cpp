...
PostsFeed::PostsFeed(Sidechain::Stores::AppStore *store) : AppStoreComponent(store) {
  ...
  commentsPanel->onUserClicked = [this](const juce::String &userId) {
    ...
    if (onNavigateToProfile && userId.isNotEmpty())
      ...
      Log::warn("PostsFeedComponent: User clicked in comments but callback not "
                "set or userId empty");
  };
...
void PostsFeed::setStreamChatClient(StreamChatClient *client) {
  ...
  Log::info("PostsFeed::setStreamChatClient: StreamChatClient set " +
            juce::String(client != nullptr ? "(valid)" : "(null)"));
}

void PostsFeed::setAudioPlayer(HttpAudioPlayer *player) {
  Log::info("PostsFeed::setAudioPlayer: Setting audio player " +
            juce::String(player != nullptr ? "(valid)" : "(null)"));
  audioPlayer = player;

  if (audioPlayer) {
    // Set up progress callback to update post cards
    audioPlayer->onProgressUpdate = [this](const juce::String &postId, double progress) {
      Log::debug("PostsFeedComponent: Audio progress update - postId: " + postId +
                 ", progress: " + juce::String(progress, 2));
    ...
    audioPlayer->onPlaybackStarted = [this](const juce::String &postId) {
      ...
      if (networkClient != nullptr) {
        ...
        if (appStore != nullptr) {
          using namespace Sidechain::Stores;
          auto currentFeedType = appStore->getState().posts.currentFeedType;
          ...
          if (source != "unknown" && position >= 0) {
            Log::debug("PostsFeedComponent: Tracking recommendation click: postId=" + postId + " source=" + source +
    ...
    audioPlayer->onLoadingComplete = [this](const juce::String &postId, bool success) {
      Log::debug("PostsFeedComponent: Audio loading complete - postId: " + postId +
...
void PostsFeed::loadFeed() {
  ...
  if (appStore) {
    appStore->loadFeed(appStore->getState().posts.currentFeedType, false);
...
void PostsFeed::switchFeedType(Sidechain::Stores::FeedType type) {
  ...
  juce::String currentTypeStr =
      feedTypeToString(appStore ? appStore->getState().posts.currentFeedType : FeedType::Timeline);

  if (appStore && appStore->getState().posts.currentFeedType == type) {
...
void PostsFeed::queryPresenceForPosts() {
  ...
  if (!streamChatClient || !appStore) {
    Log::debug("PostsFeed::queryPresenceForPosts: Skipping - streamChatClient "
               "or appStore is null");
    return;
  }

  auto currentFeed = appStore->getState().posts.getCurrentFeed();
...
void PostsFeed::drawFeedTabs(juce::Graphics &g) {
  ...
  using namespace Sidechain::Stores;
  auto currentFeedType = appStore ? appStore->getState().posts.currentFeedType : FeedType::Timeline;
  ...
  if (isTimelineActive) {
    UIHelpers::drawButton(g, timelineTab.reduced(5), "Following", SidechainColors::primary(),
  ...
  // Use UIHelpers::drawButton for consistent tab styling
  if (isTrendingActive) {
    UIHelpers::drawButton(g, trendingTab.reduced(5), "Trending", SidechainColors::primary(),
                          SidechainColors::textPrimary(), false, 4.0f);
  } else {
    g.setColour(SidechainColors::textMuted());
    g.setFont(13.0f);
  ...
  // Use UIHelpers::drawButton for consistent tab styling
  if (isGlobalActive) {
    UIHelpers::drawButton(g, globalTab.reduced(5), "Discover", SidechainColors::primary(),
                          SidechainColors::textPrimary(), false, 4.0f);
  } else {
    g.setColour(SidechainColors::textMuted());
    g.setFont(13.0f);
  ...
  // Use UIHelpers::drawButton for consistent tab styling
  if (isForYouActive) {
    UIHelpers::drawButton(g, forYouTab.reduced(5), "For You", SidechainColors::primary(),
                          SidechainColors::textPrimary(), false, 4.0f);
  } else {
    g.setColour(SidechainColors::textMuted());
    g.setFont(13.0f);
  ...
  const auto currentFeed = appStore ? appStore->getState().posts.getCurrentFeed() : nullptr;
  ...
  UIHelpers::drawDivider(g, 0, tabsBounds.getBottom(), getWidth(), SidechainColors::borderSubtle(), 1.0f);
}
...
void PostsFeed::drawLoadingState(juce::Graphics &g) {
  ...
  g.setFont(18.0f);
  g.drawText("Loading feed...", centerBounds.withY(centerBounds.getY() + 60).withHeight(30),
             juce::Justification::centred);

  g.setColour(SidechainColors::textMuted());
  g.setFont(14.0f);
  g.drawText("Fetching latest posts", centerBounds.withY(centerBounds.getY() + 95).withHeight(25),
             juce::Justification::centred);
}
...
void PostsFeed::drawEmptyState(juce::Graphics &g) {
  ...
  UIHelpers::drawButton(g, actionBtn, "Start Recording", SidechainColors::primary(), SidechainColors::textPrimary(),
                        false, 8.0f);
}
...
void PostsFeed::drawErrorState(juce::Graphics &g) {
  ...
  g.setColour(SidechainColors::textPrimary());
  g.setFont(20.0f);
  g.drawText("Couldn't Load Feed", centerBounds.withY(centerBounds.getY() + 90).withHeight(35),
             juce::Justification::centred);
  ...
  UIHelpers::drawButton(g, retryBtn, "Try Again", SidechainColors::primary(), SidechainColors::textPrimary(), false,
                        8.0f);
}
  ...
  if (appStore) {
    ...
    if (currentFeed && currentFeed->isLoading) {
      ...
      if (loadingY < contentBounds.getBottom()) {
        g.setColour(SidechainColors::textMuted());
        g.setFont(14.0f);
        g.drawText("Loading more...", contentBounds.getX(), loadingY, contentBounds.getWidth(), 40,
                   juce::Justification::centred);
      }
    }
  }
...
void PostsFeed::rebuildPostCards() {
  ...
  const auto currentFeedType = appStore->getState().posts.currentFeedType;
  ...
  const auto &fullState = appStore->getState();
  Log::debug("PostsFeed::rebuildPostCards: State has " + juce::String(fullState.posts.feeds.size()) + " feeds, " +
             juce::String(fullState.posts.aggregatedFeeds.size()) + " aggregated feeds");

  if (isAggregated) {
    // Build aggregated feed cards
    const auto aggFeed = appStore->getState().posts.getCurrentAggregatedFeed();
    if (!aggFeed)
      return;
    ...
    for (const auto &group : groups) {
      ...
      card->onPlayClicked = [this](const juce::String &playPostId) {
        ...
        const auto playAggFeed = appStore->getState().posts.getCurrentAggregatedFeed();
        if (!playAggFeed)
          return;
...
void PostsFeed::updatePostCardPositions() {
  ...
  if (!aggregatedCards.isEmpty()) {
    ...
    for (int i = 0; i < postCards.size(); ++i) {
      ...
      int cardY = contentBounds.getY() + POSTS_TOP_PADDING - static_cast<int>(scrollPosition) +
                  i * (POST_CARD_HEIGHT + POST_CARD_SPACING);
...
void PostsFeed::setupPostCardCallbacks(PostCard *card) {
                  ...
                  if (reportApiResult.isOk()) {
                    ...
                    juce::MessageManager::callAsync([]() {
                      juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::InfoIcon, "Report Submitted",
                                                             "Thank you for reporting this post. We will review "
                                                             "it shortly.");
                    });
                  } else {
  ...
  card->onAddToDAWClicked = [](const FeedPost &post) {
    ...
    auto chooser = std::make_shared<juce::FileChooser>(
        "Save audio to DAW project folder...", juce::File::getSpecialLocation(juce::File::userDocumentsDirectory),
        "*.wav,*.mp3,*.flac");

    chooser->launchAsync(juce::FileBrowserComponent::saveMode, [post, chooser](const juce::FileChooser &fc) {
      ...
      Async::runVoid([post, targetFile]() {
        ...
        if (audioUrl.readEntireBinaryStream(audioData)) {
          ...
          juce::MessageManager::callAsync([post]() {
            ...
            juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon, "Error",
                                                   "Failed to download audio file. Please check your "
  ...
  card->onDropToTrackClicked = [this, card](const FeedPost &post) {
    ...
    networkClient->getPostDownloadInfo(
        post.id, [this, post, card](Outcome<NetworkClient::DownloadInfo> downloadInfoOutcome) {
          ...
          // For now, use a generic location - we'll enhance this with
          // DAW-specific paths later
  ...
  card->onDownloadProjectClicked = [this](const FeedPost &post) {
    ...
    if (networkClient == nullptr) {
      Log::warn("PostsFeedComponent: Cannot download project file - "
                "networkClient is null");
      juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::WarningIcon, "Error",
  ...
  card->onRemixChainClicked = [this](const FeedPost &post) {
    ...
    networkClient->getRemixChain(post.id, [postId = post.id](Outcome<juce::var> result) {
      juce::MessageManager::callAsync([result, postId]() {
        ...
        juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::InfoIcon, "Remix Chain", chainText);
      });
    });
  };
...
void PostsFeed::mouseUp(const juce::MouseEvent &event) {
  ...
  if (getRefreshButtonBounds().contains(pos) && canRefresh) {
  ...
  // Note: Discover/search and profile clicks now handled by central
  // HeaderComponent
}
...
bool PostsFeed::keyPressed(const juce::KeyPress &key, juce::Component * /*originatingComponent*/) {
  ...
  // Note: Card tap to expand details is implemented - opens comments panel
  ...
  if (key == juce::KeyPress::upKey) {
    ...
    Log::debug("PostsFeed::keyPressed: Up arrow - volume " + juce::String(oldVolume, 2) + " -> " +
               juce::String(newVolume, 2));
    return true;
  }
  ...
  if (key == juce::KeyPress::downKey) {
    ...
    audioPlayer->setVolume(newVolume);
    Log::debug("PostsFeed::keyPressed: Down arrow - volume " + juce::String(oldVolume, 2) + " -> " +
               juce::String(newVolume, 2));
    return true;
  }
...
void PostsFeed::updateAudioPlayerPlaylist() {
  ...
  Log::info("PostsFeed::updateAudioPlayerPlaylist: Updating playlist - posts: " + juce::String(postIds.size()) +
            " with audio");
