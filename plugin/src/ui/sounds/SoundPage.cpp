...
void SoundPage::mouseWheelMove(const juce::MouseEvent& event, const juce::MouseWheelDetails& wheel)
{
    ...
    int newOffset = juce::jlimit(0, juce::jmax(0, calculateContentHeight() - getContentBounds().getHeight()),
                                  scrollOffset - delta);
...
void SoundPage::drawSoundInfo(juce::Graphics& g, juce::Rectangle<int>& bounds)
{
    ...
    g.setColour(Colors::accent);
    g.drawText(sound.getCreatorName().isNotEmpty() ? sound.getCreatorName() : "Unknown",
