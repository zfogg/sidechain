...
public:
    ...
    {
        return getCurrentKey();  // After finalize(), getCurrentKey() returns final result
    }
...
{
public:
    bool start(double) { return false; }
    bool addAudioChunk(const juce::AudioBuffer<float>&, int) { return false; }
    KeyDetector::Key getCurrentKey() const { return KeyDetector::Key(); }
    bool finalize() { return false; }
    KeyDetector::Key getFinalKey() const { return KeyDetector::Key(); }
    void reset() {}
};
