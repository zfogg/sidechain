  ...
  juce::AudioBuffer<float> createTestBuffer(int numSamples, float amplitude = 0.5f) {
    ...
    for (int ch = 0; ch < 2; ++ch) {
      ...
      for (int i = 0; i < numSamples; ++i) {
        ...
        data[i] =
            amplitude * std::sin(2.0f * juce::MathConstants<float>::pi * 440.0f * static_cast<float>(i) / 44100.0f);
      }
    }
    return buffer;
  }
...
TEST_CASE_METHOD(AudioCaptureTestFixture, "AudioCapture waveform generation", "[AudioCapture]") {
  SECTION("generateWaveformSVG produces valid SVG") {
    ...
    for (int i = 0; i < 1024; ++i) {
      float sample = std::sin(2.0f * juce::MathConstants<float>::pi * static_cast<float>(i) / 100.0f);
...
TEST_CASE_METHOD(AudioCaptureTestFixture, "AudioCapture WAV export", "[AudioCapture][Export]") {
  ...
  SECTION("saveBufferToWavFile saves valid WAV file") {
    ...
    bool success =
        AudioCapture::saveBufferToWavFile(tempFile, testBuffer, 44100.0, AudioCapture::ExportFormat::WAV_16bit);
  ...
  SECTION("saveBufferToWavFile supports different bit depths") {
    ...
    REQUIRE(AudioCapture::saveBufferToWavFile(file16, testBuffer, 44100.0, AudioCapture::ExportFormat::WAV_16bit));
    auto size16 = file16.getSize();
    file16.deleteFile();

    // Test 24-bit
    auto file24 = AudioCapture::getTempAudioFile(".wav");
    REQUIRE(AudioCapture::saveBufferToWavFile(file24, testBuffer, 44100.0, AudioCapture::ExportFormat::WAV_24bit));
  ...
  SECTION("saveBufferToWavFile fails with empty buffer") {
    ...
    bool success = AudioCapture::saveBufferToWavFile(tempFile, emptyBuffer, 44100.0);
  ...
  SECTION("saveBufferToWavFile fails with invalid sample rate") {
    ...
    bool success = AudioCapture::saveBufferToWavFile(tempFile, testBuffer, 0.0);
...
TEST_CASE_METHOD(AudioCaptureTestFixture, "AudioCapture FLAC export", "[AudioCapture][Export][FLAC]") {
  ...
  SECTION("saveBufferToFlacFile saves valid FLAC file") {
    ...
    bool success =
        AudioCapture::saveBufferToFlacFile(tempFile, testBuffer, 44100.0, AudioCapture::ExportFormat::FLAC_16bit);
  ...
  SECTION("FLAC is smaller than WAV for same audio") {
    ...
    REQUIRE(AudioCapture::saveBufferToWavFile(wavFile, testBuffer, 44100.0, AudioCapture::ExportFormat::WAV_16bit));
    auto wavSize = wavFile.getSize();

    auto flacFile = AudioCapture::getTempAudioFile(".flac");
    REQUIRE(AudioCapture::saveBufferToFlacFile(flacFile, testBuffer, 44100.0, AudioCapture::ExportFormat::FLAC_16bit));
  ...
  SECTION("saveBufferToFile routes to correct format") {
    ...
    // Test WAV routing
    auto wavFile = AudioCapture::getTempAudioFile(".wav");
    REQUIRE(AudioCapture::saveBufferToFile(wavFile, testBuffer, 44100.0, AudioCapture::ExportFormat::WAV_16bit));
    REQUIRE(wavFile.exists());
    wavFile.deleteFile();

    // Test FLAC routing
    auto flacFile = AudioCapture::getTempAudioFile(".flac");
    REQUIRE(AudioCapture::saveBufferToFile(flacFile, testBuffer, 44100.0, AudioCapture::ExportFormat::FLAC_16bit));
  ...
  SECTION("saveBufferToFlacFile supports different bit depths") {
    ...
    REQUIRE(AudioCapture::saveBufferToFlacFile(file16, testBuffer, 44100.0, AudioCapture::ExportFormat::FLAC_16bit));
    auto size16 = file16.getSize();
    file16.deleteFile();

    // 24-bit FLAC
    auto file24 = AudioCapture::getTempAudioFile(".flac");
    REQUIRE(AudioCapture::saveBufferToFlacFile(file24, testBuffer, 44100.0, AudioCapture::ExportFormat::FLAC_24bit));
    auto size24 = file24.getSize();
    file24.deleteFile();
  ...
  SECTION("saveBufferToFlacFile fails with empty buffer") {
    ...
    bool success = AudioCapture::saveBufferToFlacFile(tempFile, emptyBuffer, 44100.0);
...
TEST_CASE("AudioCapture channel configurations", "[AudioCapture][Channels]") {
  ...
  SECTION("mono configuration (1 channel)") {
    ...
    for (int i = 0; i < 1024; ++i) {
      monoBuffer.setSample(
          0, i, 0.5f * std::sin(2.0f * juce::MathConstants<float>::pi * 440.0f * static_cast<float>(i) / 44100.0f));
    }
