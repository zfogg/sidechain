...
export class FeedPostModel {
  ...
  static fromJson(json: FeedPostJSON): FeedPost {
    ...
    return {
      ...
      durationSeconds: parseFloat(json.duration_seconds || json.duration || '0'),
      bpm: parseInt(json.bpm || '0', 10),
      key: json.key || '',
      daw: json.daw || '',
      genre: Array.isArray(json.genre) ? json.genre : [],

      likeCount: parseInt(json.reaction_counts?.like || json.like_count || '0', 10),
      playCount: parseInt(json.play_count || json.playback_count || '0', 10),
      commentCount: parseInt(json.comment_count || json.comments_count || '0', 10),
      saveCount: parseInt(json.save_count || json.saved_count || '0', 10),
      repostCount: parseInt(json.repost_count || json.reposts_count || '0', 10),

      isLiked: json.own_reactions?.like?.length > 0 || json.is_liked || false,
